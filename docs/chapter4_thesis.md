...
本节依照4.1所确立的两域分治架构，逐模块阐述各功能单元的具体实现方案。软件服务域方面，重点介绍后端推理调度逻辑与前后端交互机制；模型推理域方面，依次说明语音合成、视频渲染、多模态特征提取与视频质量评估四个模块的功能实现原理与核心处理流程。

---

### 4.2.1 语音合成模块实现

语音合成模块基于GPT-SoVITS框架构建，属于端到端的文本转语音（TTS）系统，具备零样本声音克隆能力。该框架由GPT语言模型与SoVITS声码器两级模型协同完成推理，前者负责将文本序列转换为语音语义令牌（semantic token），后者负责将语义令牌与参考音频的音色特征融合，最终合成目标语音波形。其整体推理流程如图4-4所示。
...
```
  ...
  │  参考音频（Ref_Wav.wav，目标说话人音色）                        │
  │  参考音频对应文字（Ref_text.json）                             │
  └──────────────────────────────┬──────────────────────────────┘
                                 │
                                 ▼
  ┌──────────────────────────────────────────────────────────────┐
  │  第一步：文本预处理（LangSegment）                              │
  │  · 多语言混合分词（中/英/日/韩自动识别）                         │
  │  · 按标点/字数切分为适合推理的短句序列                           │
  └──────────────────────────────┬───────────────────────────────┘
                                 │
                                 ▼
  ┌──────────────────────────────────────────────────────────────┐
  │  第二步：GPT语言模型（自回归文本到语义令牌）                       │
  │  · 输入：目标文本 token + 参考音频特征                           │
  │  · 输出：逐帧语音语义令牌序列（speaker-conditioned）             │
  └──────────────────────────────┬───────────────────────────────┘
                                 │
                                 ▼
  ┌──────────────────────────────────────────────────────────────┐
  │  第三步：SoVITS声码器（语义令牌 → 音频波形）                      │
  │  · 输入：语义令牌序列 + 参考音频的音色向量（VQ编码）               │
  │  · 输出：与参考说话人音色一致的目标语音 WAV 文件                   │
  └──────────────────────────────┬───────────────────────────────┘
                                 │
                                 ▼
  ┌──────────────────────────────────────────────────────────────┐
  │  第四步：音频写出与路径记录                                      │
  │  · 按幻灯片页码分帧写出 WAV 文件至 PPT_Audio/ 目录               │
  │  · 将页码→音频路径映射写入 Audio_save_path.json                 │
  └──────────────────────────────────────────────────────────────┘
```

#### 4.2.1.1 文本预处理与多语言切分

在将文本送入GPT语言模型之前，系统调用LangSegment工具对输入文本进行语言检测与分句处理。LangSegment依据Unicode字符范围自动识别中文、英文、日文、韩文等多语言混合文本，将连续的句子按标点符号或预设字数阈值切分为适合单次推理的短句序列，避免过长句子导致注意力机制建模质量下降。切分后的句子列表以迭代方式逐条送入TTS推理流程，各页幻灯片对应的备注文本经此流程生成独立WAV音频文件。

#### 4.2.1.2 GPT-SoVITS声音克隆推理
...
系统通过`change_gpt_weights`与`change_sovits_weights`两个函数接口分别热加载GPT和SoVITS的模型权重，支持在运行时切换不同的声音克隆模型，无需重启后端服务。推理时调用`get_tts_wav`函数接口，传入参考音频路径、参考文本、目标文本及切句策略，以生成器形式逐帧输出PCM音频数据，后端代码消费该生成器直至迭代完成，取最后一帧完整音频写入磁盘。

#### 4.2.1.3 批量推理与音频管理

系统为每张幻灯片的备注文本独立发起一次TTS推理，所有生成的WAV文件按页码序号命名，集中存放于用户任务目录下的`PPT_Audio`子目录。推理完成后，系统遍历该目录并将页码到文件路径的映射以JSON格式写入`Audio_save_path.json`，作为后续渲染模块的音频输入清单。若用户提供了自录音频（`User_Wav.wav`），则跳过TTS环节，直接将自录音频注册至音频清单，沿Wav2Lip渲染路径处理。

---

### 4.2.2 视频渲染模块实现

视频渲染模块将语音合成阶段产出的音频与参考人脸图像合成为具有自然唇形与面部表情的数字人说话视频。依据音频来源，系统提供两条互斥的渲染路径：路径A（用户自录音频）调用Wav2Lip模型执行唇形同步渲染；路径B（TTS合成音频）调用SadTalker模型执行表情动画渲染。两条路径的渲染结果均为MP4格式视频文件，作为质量评估子系统的输入。

#### 4.2.2.1 Wav2Lip唇形同步渲染
...
多模态特征提取模块是视频质量评估子系统的前置数据处理阶段，负责从生成视频中并行提取四路异质特征：视觉深度特征、面部关键点特征、面部动作单元特征与音频语义特征，为bs质量评估模型提供标准化的多模态输入张量。其处理流程如图4-5所示。
...
```
  输入：MP4 视频文件
  ┌───────────────────────────────────────────────────────────┐
  │  视频解码（OpenCV，25fps）        音频分离（ffmpeg，16kHz WAV）  │
  └─────────────────────┬─────────────────────┬───────────────┘
                        │ 视频帧序列             │ 音频波形
        ┌───────────────┼─────────────┐        │
        ↓               ↓             ↓        ↓
  ┌───────────┐  ┌───────────┐  ┌──────────┐  ┌──────────────┐
  │ 路径A     │  │ 路径B     │  │ 路径C    │  │ 路径D        │
  │ 视觉深度  │  │ 面部关键点 │  │ 面部AU   │  │ 音频语义     │
  │ 特征提取  │  │ 特征提取  │  │ 特征提取  │  │ 特征提取     │
  │           │  │           │  │          │  │              │
  │RetinaFace │  │MediaPipe  │  │py-feat   │  │HuBERT        │
  │人脸检测   │  │Face Mesh  │  │SVM AU    │  │768维语义     │
  │ResNet-101 │  │468关键点  │  │检测器    │  │特征序列      │
  │2048维特征 │  │1404维向量 │  │17维AU值  │  │              │
  └─────┬─────┘  └─────┬─────┘  └────┬─────┘  └──────┬───────┘
        └───────────────┴─────────────┴────────────────┘
                                      │
                                      ▼
...
视觉深度特征提取模块（`VisualFeatureExtractor`）采用py-feat工具链中的RetinaFace检测器对每帧图像进行人脸定位，在检测到有效人脸后裁剪并归一化至224×224分辨率，送入预训练的ResNet-101卷积神经网络进行特征编码。ResNet-101移除最后的全连接分类层后，提取最终全局平均池化层输出的2048维特征向量，作为该帧的视觉深度特征表示。该特征向量整合了人脸的全局外观信息，包含肤色分布、五官形态、光照条件与整体面部结构等维度的高层语义特征，是评估表情自然度与视频整体质量的重要依据。

#### 4.2.3.2 面部关键点特征提取

面部关键点特征提取模块（`KeypointFeatureExtractor`）采用MediaPipe Face Mesh检测器，对视频的每一帧提取468个三维面部关键点，每个关键点包含归一化坐标三元组（x, y, z），展开后形成1404维的关键点向量。468个关键点涵盖面部轮廓、眉毛、鼻梁、眼睛（含上下眼睑、眼角及虹膜区域）和嘴唇（含内外唇廓）等全部区域，其中眼周关键点（索引33至133）密集分布，能够精细描述眼睑开合状态与注视方向。关键点特征以帧级序列形式存储，携带面部几何形状的时序动态信息，是评估口型同步与表情一致性的细粒度依据。

#### 4.2.3.3 面部动作单元特征提取

面部动作单元特征提取模块（`AUFeatureExtractor`）依据面部动作编码系统（Facial Action Coding System，FACS），采用py-feat内置的SVM分类器检测面部肌肉运动的量化强度。本系统提取17维AU特征，涵盖眼睑收紧（AU46）、内眉上扬（AU01）、外眉上扬（AU02）、眨眼（AU45）、嘴角上拉（AU12）、嘴角下拉（AU15）等与表情状态高度相关的动作单元。AU特征以帧级强度值序列表示，与关键点特征互为补充：关键点提供几何位置变化，AU特征提供肌肉动作的语义类别标注，二者联合为质量评估模型提供丰富的面部运动描述。

#### 4.2.3.4 音频语义特征提取

音频语义特征提取模块（`AudioFeatureExtractor`）采用HuBERT（Hidden-Unit BERT）模型进行语音特征编码。输入音频以16000Hz采样率进行预处理，经HuBERT编码器的卷积特征提取层与Transformer上下文建模层处理后，输出768维的隐藏状态序列。HuBERT以掩码预测任务在大规模无标注语音数据上进行自监督预训练，其编码特征不仅包含基频、共振峰等底层声学属性，还编码了音素级别的语义信息与韵律节奏模式，是评估音频质量与跨模态视听一致性的核心特征来源。系统在服务启动时一次性加载HuBERT模型权重，后续每次视频评估直接调用已加载权重进行前向计算。

#### 4.2.3.5 特征序列对齐与标准化

由于视频帧率（25fps）与HuBERT输出帧率存在差异，四路特征序列的时间步数不完全一致。提取完成后，系统将各模态特征序列统一截断或零填充至最大序列长度150帧，并按批次维度对每路特征独立执行均值方差标准化，消除不同特征来源之间的数值量纲差异，保证后续Transformer编码器的数值稳定性与收敛效率。

---

### 4.2.4 视频质量评估模块实现

视频质量评估模块以bs多任务学习评估模型为核心，在数字人视频渲染完成后由后端服务自动触发，对生成视频执行多维度量化评分，并将评分结果持久化至用户任务目录，由前端在视频详情页展示并触发参数优化建议。其调用与推理流程如图4-6所示。

图4-6　视频质量评估模块调用流程图

```
  触发条件：视频渲染线程完成，MP4 文件写入用户任务目录
                             │
                             ▼
  ┌─────────────────────────────────────────────────────────┐
  │  步骤1：后端服务调用评估入口                               │
  │  evaluate_single_video(video_path, checkpoint, config)   │
  └────────────────────────────┬────────────────────────────┘
                               │
                               ▼
  ┌─────────────────────────────────────────────────────────┐
  │  步骤2：多模态特征提取（调用 4.2.3 节所述模块）              │
  │  FeatureExtractor.extract_all_features(video_path)       │
  │  → {"visual": [T×2048], "audio": [T×768],               │
  │     "keypoint": [T×1404], "au": [T×17]}                 │
  └────────────────────────────┬────────────────────────────┘
                               │
                               ▼
  ┌─────────────────────────────────────────────────────────┐
  │  步骤3：特征张量预处理                                     │
  │  · 各模态 numpy → torch.Tensor，增加批次维度               │
  │  · 截断/填充至150帧，统一设备（GPU/CPU）                    │
  └────────────────────────────┬────────────────────────────┘
                               │
                               ▼
  ┌─────────────────────────────────────────────────────────┐
  │  步骤4：bs 模型前向推理（eval模式，torch.no_grad()）         │
  │  TalkingFaceEvaluationModel.forward(                     │
  │      visual, audio, keypoint, au_features)               │
  │  → {"lip_sync": x.xx, "expression": x.xx,               │
  │     "audio_quality": x.xx, "cross_modal": x.xx,          │
  │     "overall": x.xx}                                     │
  └────────────────────────────┬────────────────────────────┘
                               │
                               ▼
  ┌─────────────────────────────────────────────────────────┐
  │  步骤5：结果持久化与返回                                    │
  │  · 评分字典序列化为 JSON，写入用户任务目录 scores.json        │
  │  · 后端将评分随视频文件路径一并响应前端拉取请求                │
  └─────────────────────────────────────────────────────────┘
```

#### 4.2.4.1 bs模型架构与多任务前向推理

bs模型（`TalkingFaceEvaluationModel`）采用多任务学习框架，由四路独立的模态编码器、一个跨模态融合编码器与五个任务预测头构成。其架构如图4-7所示。

图4-7　bs多任务评估模型内部结构示意图

```
  输入特征（各模态独立编码）
  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐
  │视觉特征   │  │音频特征   │  │关键点特征  │  │AU特征    │
  │[B,T,2048]│  │[B,T,768] │  │[B,T,1404]│  │[B,T,17]  │
  └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘
       ↓              ↓              ↓              ↓
  ┌────────┐    ┌────────┐    ┌────────┐    ┌────────┐
  │视觉线性 │    │音频线性 │    │关键点  │    │AU线性  │
  ...
  │                      五个任务预测头                     │
  ├──────────┬──────────┬──────────┬──────────┬──────────┤
  │ 口型同步  │ 表情自然度│ 音频质量  │跨模态    │ 综合质量  │
  │  head    │  head    │  head    │一致性head │  head    │
  │  → [B,1] │  → [B,1] │  → [B,1] │  → [B,1] │  → [B,1] │
  └──────────┴──────────┴──────────┴──────────┴──────────┘
```

各模态编码器首先通过独立的线性投影层将各自维度的输入特征对齐至统一的隐层维度`d_model`。对齐后的四路特征在时间步维度拼接或加和，经多层Transformer编码器建模帧间时序依赖与跨模态语义关联。Transformer编码器采用标准的多头自注意力机制，使每个时间步的隐状态能够关注其他所有时间步及其他模态的特征，从而捕获音视频对齐偏差、表情连续性等时序质量信号。

编码完成后，对时间步维度执行全局平均池化，得到整段视频的固定维度特征向量，再分别送入五个独立的全连接预测头，输出对应维度的量化评分。五个预测头共享编码器的特征表示，通过多任务联合训练提升各维度的泛化能力，同时避免单任务训练时的过拟合倾向。

推理时，模型切换至评估模式（`model.eval()`），并在`torch.no_grad()`上下文中执行前向传播，跳过Dropout正则化并关闭自动微分图构建，降低推理延迟与显存占用。

#### 4.2.4.2 质量反馈闭环机制

评分完成后，后端将五维评分字典序列化为JSON格式写入用户任务目录的`scores.json`文件，与同目录下的MP4视频文件并列存储。当前端调用`/Pull_Video_Merge`接口拉取视频时，后端一并将评分数据封装于响应体内返回。前端在视频详情页通过进度条卡片可视化展示五维评分（绿色≥4.0，黄色3.0–4.0，红色<3.0），并针对低于预设阈值的维度自动展示对应的参数优化建议，具体建议规则如表4-2所示。
...
用户参照建议调整参数配置并重新触发生成，形成"生成—评估—建议—调整—再生成"的完整质量改进闭环，使视频感知质量在迭代中逐步收敛至较高水平。
