...
依据上述三项原则，系统整体采用**两域分治**的架构思路，将全部功能模块划分为**软件服务域**与**模型推理域**两大功能域。软件服务域负责用户交互与业务编排，涵盖前端交互层与后端服务层；模型推理域负责AI计算，涵盖视频生成子系统（VITS语音合成、Wav2Lip唇形渲染、SadTalker表情动画）与质量评估子系统（bs多任务视频质量评估模型）。两域之间通过后端服务层统一协调，形成"用户操作触发—软件层调度—模型层推理—结果评估回传"的完整运作链路。系统总体架构如图4-1所示。

图4-1　数字人生成系统总体架构图

（图示说明：图4-1为纵向两域分治架构图。上半部分为软件服务域，内含前端交互层与后端服务层两个水平分层，层间以双向箭头标注"HTTP/JSON通信"；下半部分为模型推理域，左侧为视频生成子系统（包含VITS、Wav2Lip、SadTalker三个模型节点及其串行连接箭头），右侧为质量评估子系统（bs模型，包含特征提取、Transformer编码、多任务评分三个节点）；两个子系统之间以单向箭头标注"MP4视频输入"，模型推理域与软件服务域之间以双向箭头标注"推理调用/结果返回"。）
...
依据两域分治的架构设计思想，系统将功能模块划分为软件服务层与模型推理层两大层次。软件服务层聚焦于用户交互与业务编排，模型推理层专注于AI模型计算与质量评估，二者职责明确、边界清晰。
...
模型推理层承载系统的全部AI计算能力，内部划分为**视频生成子系统**与**质量评估子系统**两个功能子系统。二者在执行时序上串联：视频生成子系统负责将文本与人脸图像合成为数字人说话视频，质量评估子系统随后对生成视频进行多维度自动化评分。其内部结构如下所示：
...
**视频生成子系统**集成三个协同工作的深度学习模型，形成完整的视频生成处理链路。**VITS语音合成模型**（Variational Inference with adversarial learning for end-to-end Text-to-Speech）接收讲稿文本作为输入，输出自然语音WAV音频文件，支持基于参考音频的声音克隆以还原目标说话人音色。**Wav2Lip唇形同步渲染模型**接收音频文件与参考人脸图像，采用音视频联合判别器训练策略，生成唇形与音频高度同步的数字人视频，适合对唇形精度要求较高的场景。**SadTalker表情动画渲染模型**以三维面部系数（3DMM）对面部动态进行建模，将音频特征映射为表情系数与姿态系数，经人脸渲染网络输出具有自然表情与头部姿态变化的数字人视频，适合以VITS合成语音为输入的场景。系统依据用户输入的音频配置自动选择渲染路径：若用户提供自录音频则进入Wav2Lip链路，否则进入VITS→SadTalker链路。

**质量评估子系统**集成了本系统的核心自研模型——bs多任务学习视频质量评估模型。该模型以生成视频文件为输入，通过视觉深度特征、音频语义特征、面部关键点特征与面部动作单元特征的并行提取，经跨模态Transformer编码器建模多模态时序依赖，从口型同步、表情自然度、音频质量、跨模态一致性与综合质量五个维度输出量化评分，为系统的质量反馈闭环提供数据支撑。
...
| 软件服务域 | 并发任务管理 | ThreadPoolExecutor | 适合计算与IO混合型任务，并发数可配置，避免长时推理阻塞主服务线程 |
| 模型推理域 | 语音合成 | VITS（端到端TTS） | 端到端架构，音质自然，支持参考音频声音克隆 |
...
| 模型推理域 | 音频特征提取 | HuBERT | 自监督大规模预训练，语音语义表征能力强，适合跨模态质量评估 |
...
**调用流程。** 后端服务层在接收到视频生成子系统任务完成的通知后，向质量评估子系统（bs模型）发起调用，传入已写出的视频文件路径；bs模型依次执行视频解码、多模态特征提取与Transformer前向推理，生成五维评分字典；后端将评分数据与视频文件路径信息整合，在前端发起视频拉取请求时一并响应，完成质量信息的完整闭环传递。整个调用过程对用户透明，无需额外操作。
...
（图示说明：图4-2为纵向三阶段数据流程图。上方标注"用户操作（前端）"，经带箭头连线向下进入第一阶段"请求下行路径"——前端将人脸图像、音频、文本及配置参数封装为HTTP请求发送至后端，后端解析并路由至对应推理链路，立即返回任务标识。第二阶段"推理处理路径"包含一个菱形判断节点（自录音频/参考音频路由分支），左分支指向"VITS→SadTalker"链路节点，右分支指向"Wav2Lip"链路节点，两条链路汇合后连接"bs质量评估"节点，该节点输出"五维评分JSON"。第三阶段"结果上行路径"——后端更新状态为"已完成"，前端轮询后发起拉取请求，后端返回视频文件与评分JSON，前端渲染展示并触发优化建议，形成回路箭头指向起点。）
...
（图示说明：图4-3为嵌套矩形部署示意图。最外层矩形标注"GPU工作站（单机部署）"。内部分为左右两个并列矩形区域：左侧为"前端应用区"，标注"Vue 3前端服务，监听端口5173"；右侧为"后端应用区"，标注"Flask后端服务，监听端口5000"。左右两区以双向水平箭头相连，标注"HTTP通信（localhost）"。右侧后端应用区向下延伸包含两个子区域：上方子区"视频生成子系统"内水平排列VITS、Wav2Lip、SadTalker三个模块节点；下方子区"质量评估子系统（bs模型）"内列出特征提取、Transformer编码、多任务评分三个节点。两个子区底部共同连接"用户任务目录（本地文件系统）"存储节点，以双向箭头标注"读写文件（MP4 / WAV / JSON）"。）
---
