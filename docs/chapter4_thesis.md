...
bs模型（详见第三章）在本系统中扮演"在线评委"角色：数字人视频渲染线程完成后，后端服务自动调用bs模型对生成的MP4视频进行多维度量化评分，评分结果随即通过前端界面反馈给用户，并驱动参数优化建议，形成"生成—评估—建议—调整—再生成"的完整质量改进闭环。其系统集成位置如图4-5所示。
...
用户参照建议调整参数配置并重新触发生成，形成完整的质量改进闭环，使视频感知质量在迭代中逐步收敛至较高水平。
...
| TC-V-02 | SadTalker表情动画渲染 | 测试人脸图片 + VITS合成音频，触发`/Get_Inference`（TTS路由） | 生成视频文件存在，时长与文本长度匹配 | ✅ 通过 |
| TC-V-03 | PPT与数字人视频合成 | 已有数字人MP4 + 20页PPT备注文本，触发`/PPT_Video_Merge` | 输出合成视频，各页数字人位置与PPT内容对应正确 | ✅ 通过 |
| TC-V-04 | 完整推理链路端到端 | VITS合成→SadTalker渲染完整流程，标准测试文本 | 全流程无异常中断，输出有效MP4文件 | ✅ 通过 |
