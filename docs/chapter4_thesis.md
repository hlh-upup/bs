...
【插图占位：图4-1 数字人生成系统总体四层架构图

图示说明（纵向分层矩形框架，由上至下共四层，层间以垂直带箭头连线相连）：

第一层——前端交互层（浅蓝色背景）：
  包含六个矩形模块，横向均匀排列：
  [LoginView 登录页] | [DashboardView 控制台] | [VideoConfigView 视频配置页] | [VideoListView 视频列表页] | [PersonManagerView 数字人管理页] | [VoiceTrainerView 声音训练页]
  层内注释：Vue 3 + TypeScript + Pinia + Vue Router

第二层——后端服务层（浅绿色背景）：
  包含四个矩形模块，横向排列：
  [Flask RESTful API 路由调度] | [ThreadPoolExecutor 线程池（max_workers=5）] | [用户文件目录管理] | [任务状态机（Processing/True/Failed）]
  层内注释：Python Flask + Flask-CORS

第三层——模型推理层（浅橙色背景）：
  包含三个矩形模块，横向排列：
  [VITS 端到端语音合成] | [Wav2Lip 唇形同步渲染] | [SadTalker 3DMM表情动画]
  层内注释：PyTorch，GPU加速推理

第四层——质量评估层（浅紫色背景）：
  包含三个矩形模块，横向排列：
  [多模态特征提取器（视觉/音频/关键点/AU）] | [bs 多任务质量评估模型] | [评分JSON写入与持久化]
  层内注释：Transformer + 多任务学习

层间连接箭头（双向或单向）：
  前端交互层 ←──HTTP RESTful JSON──→ 后端服务层
  后端服务层 ──Python函数调用──→ 模型推理层
  模型推理层 ──MP4视频文件──→ 质量评估层
  质量评估层 ──评分JSON──→ 后端服务层 ──响应──→ 前端交互层】
...
【插图占位：图4-2 数字人生成系统单机部署架构图

图示说明（带标注的嵌套矩形框架图）：

最外层矩形：标题"GPU工作站服务器（本地部署环境）"

内部分为左右两个并列主框：

左框——前端服务区：
  矩形标题：[Vue 3 前端应用 / Vite Dev Server]
  内部标注：监听端口 5173 / HTTP 服务

右框——后端服务区：
  矩形标题：[Flask 后端 API / server1.4.0.py]
  内部标注：监听端口 5000 / Python 进程

左框与右框之间：双向水平箭头，标注：
  "HTTP 跨域请求（CORS / localhost）"
  "请求体：JSON / Base64 / multipart"

右框内部向下延伸，包含两个子框：

子框A——模型推理区：
  四个并列模块（横向排列）：
  [VITS 语音合成 / GPU:CUDA] | [Wav2Lip 唇形渲染 / GPU:CUDA] | [SadTalker 表情动画 / GPU:CUDA] | [bs 质量评估 / GPU:CUDA]
  右框→子框A连线标注："Python函数调用 / 子线程（ThreadPoolExecutor）"

子框B——文件系统存储区：
  标题：[用户任务目录 / 本地文件系统]
  内含标注：用户图片 / 参考音频 / 合成视频 / 评分JSON
  子框A与子框B之间双向箭头标注："读写文件（MP4/WAV/JSON）"】
...
页面节点及跳转关系：
  ...
  [视频列表 VideoListView]
    ──（点击某条记录"查看详情"）──→ [视频详情+评分页面]（含图4-6所示的五维评分卡片）
    ──（点击"下载"）──→ 浏览器文件下载
