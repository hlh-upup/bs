...
本节依照4.1节所确立的两域分治架构，逐模块阐述各功能单元的具体实现方案。模型推理域方面，4.2.1节以"模型驱动数字人视频生成"为主题，统一介绍语音合成与视频渲染两个紧密衔接的子阶段；4.2.2节重点阐述视频质量评估模块，涵盖多模态特征提取与bs模型推理两个核心环节；软件服务域方面，4.2.3节说明前后端交互与推理调度机制。
...
视频质量评估模块以bs多任务学习评估模型为核心，在数字人视频渲染完成后由后端服务自动触发，对生成视频执行多维度量化评分，并将评分结果持久化至用户任务目录，由前端在视频详情页展示并触发参数优化建议。该模块由两个紧密衔接的子环节组成：前置的多模态特征提取子模块负责从生成视频中并行抽取视觉、音频、关键点与动作单元四路异质特征；随后的bs模型推理子模块将多模态特征张量送入多任务评估网络，输出五维量化评分。其整体调用流程如图4-5所示。
...
```
  ...
  └────────────────────────────┬────────────────────────────┘
                               ...
                               ▼
  ┌─────────────────────────────────────────────────────────┐
  │  步骤2：多模态特征提取（调用本节4.2.2.1所述特征提取子模块）    │
...
多模态特征提取子模块负责从生成视频中并行提取四路异质特征，为bs评估模型提供标准化的多模态输入张量。模块对视频进行逐帧解码，同时以ffmpeg分离音频轨道，随后将视频帧序列与音频波形分别送入四条并行处理路径，如图4-6所示。
...
**视觉深度特征提取。** 视觉深度特征提取模块（`VisualFeatureExtractor`）采用py-feat工具链中的RetinaFace检测器对每帧图像进行人脸定位，在检测到有效人脸后裁剪并归一化至224×224分辨率，送入预训练的ResNet-101卷积神经网络进行特征编码。ResNet-101移除最后的全连接分类层后，提取最终全局平均池化层输出的2048维特征向量，作为该帧的视觉深度特征表示。该特征向量整合了人脸的全局外观信息，包含肤色分布、五官形态、光照条件与整体面部结构等维度的高层语义特征，是评估表情自然度与视频整体质量的重要依据。

**面部关键点特征提取。** 面部关键点特征提取模块（`KeypointFeatureExtractor`）采用MediaPipe Face Mesh检测器，对视频的每一帧提取468个三维面部关键点，每个关键点包含归一化坐标三元组（x, y, z），展开后形成1404维的关键点向量。468个关键点涵盖面部轮廓、眉毛、鼻梁、眼睛（含上下眼睑、眼角及虹膜区域）和嘴唇（含内外唇廓）等全部区域，其中眼周关键点（索引33至133）密集分布，能够精细描述眼睑开合状态与注视方向。关键点特征以帧级序列形式存储，携带面部几何形状的时序动态信息，是评估口型同步与表情一致性的细粒度依据。

**面部动作单元特征提取。** 面部动作单元特征提取模块（`AUFeatureExtractor`）依据面部动作编码系统（Facial Action Coding System，FACS），采用py-feat内置的SVM分类器检测面部肌肉运动的量化强度。本系统提取17维AU特征，涵盖眼睑收紧（AU46）、内眉上扬（AU01）、外眉上扬（AU02）、眨眼（AU45）、嘴角上拉（AU12）、嘴角下拉（AU15）等与表情状态高度相关的动作单元。AU特征以帧级强度值序列表示，与关键点特征互为补充：关键点提供几何位置变化，AU特征提供肌肉动作的语义类别标注，二者联合为质量评估模型提供丰富的面部运动描述。

**音频语义特征提取。** 音频语义特征提取模块（`AudioFeatureExtractor`）采用HuBERT（Hidden-Unit BERT）模型进行语音特征编码。输入音频以16000Hz采样率进行预处理，经HuBERT编码器的卷积特征提取层与Transformer上下文建模层处理后，输出768维的隐藏状态序列。HuBERT以掩码预测任务在大规模无标注语音数据上进行自监督预训练，其编码特征不仅包含基频、共振峰等底层声学属性，还编码了音素级别的语义信息与韵律节奏模式，是评估音频质量与跨模态视听一致性的核心特征来源。系统在服务启动时一次性加载HuBERT模型权重，后续每次视频评估直接调用已加载权重进行前向计算，避免重复初始化开销。

**特征序列对齐与标准化。** 由于视频帧率（25fps）与HuBERT输出帧率存在差异，四路特征序列的时间步数不完全一致。提取完成后，系统将各模态特征序列统一截断或零填充至最大序列长度150帧，并按批次维度对每路特征独立执行均值方差标准化，消除不同特征来源之间的数值量纲差异，保证后续Transformer编码器的数值稳定性与收敛效率。

#### 4.2.2.2 bs模型架构与多任务前向推理
