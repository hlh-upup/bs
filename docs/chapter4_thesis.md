...
由于视频帧率（25fps）与音频帧率不同，各模态特征序列的时间步长存在差异。提取完成后，各模态特征序列被统一截断或填充至最大序列长度150帧，并进行批次维度的标准化处理（减均值除方差），以消除不同特征来源之间的数值尺度差异，提升模型训练的数值稳定性。

---

### 4.2.3 注意力检测模块实现

注意力检测模块（即评估模型）是本系统的核心创新模块，采用多任务学习（Multi-Task Learning，MTL）与跨模态Transformer注意力机制，对数字人视频进行多维度质量感知推断。该模块以面部特征模块输出的多模态特征序列为输入，通过双路径融合编码与多任务预测头，同时输出五个维度的质量评分，实现对生成视频注意力相关特征（眼部状态、唇形同步、表情自然度）的自动化量化评估。

【插图占位：图4-5 评估模型整体架构图（四模态输入 → 残差编码 → 时序建模 → 双路径融合 → 五任务输出）】

#### 4.2.3.1 模型输入与特征对齐

模型接收四路特征序列作为输入，形状均为 $\mathbf{F} \in \mathbb{R}^{B \times T \times D_i}$，其中 $B$ 为批次大小，$T$ 为序列时间步长，$D_i$ 分别为各模态的原始特征维度（视觉2048维、音频768维、关键点1404维、AU 17维）。各模态首先经过自适应线性投影层，将异构特征维度统一映射至统一的编码器维度 $D$（256维），完成跨模态维度对齐：

$$\mathbf{h}_i = \mathbf{W}_i \mathbf{F}_i + \mathbf{b}_i, \quad \mathbf{h}_i \in \mathbb{R}^{B \times T \times D}$$

#### 4.2.3.2 残差MLP特征编码

维度对齐后，每一模态特征经由独立的三层残差MLP模块进行局部非线性变换，提取模态内部的高层语义表示：

$$\mathbf{h}_i' = \mathbf{h}_i + \text{GELU}(\text{LayerNorm}(\mathbf{W}_3 \cdot \text{GELU}(\text{LayerNorm}(\mathbf{W}_2 \cdot \text{GELU}(\text{LayerNorm}(\mathbf{W}_1 \mathbf{h}_i))))))$$

其中残差连接保证了梯度的稳定传播，Layer Normalization与Dropout（丢弃率0.3）提升了训练的正则化效果。

#### 4.2.3.3 时序Transformer建模

各模态编码后的特征序列经由共享的时序建模模块（TemporalBlock）进行帧间依赖关系的捕捉。TemporalBlock由多头自注意力机制与前馈网络组成，包含双重残差连接：

$$\text{Attn}(\mathbf{Q}, \mathbf{K}, \mathbf{V}) = \text{softmax}\left(\frac{\mathbf{Q}\mathbf{K}^\top}{\sqrt{D/H}}\right)\mathbf{V}$$

其中 $H=8$ 为注意力头数，$D/H=32$ 为每头维度。注意力机制能够自适应地对序列中的关键时间步赋予更高权重，从而在不规则的眼部运动或唇形抖动序列中定位异常帧。时序建模完成后，对序列维度分别进行均值池化（Mean Pooling）与最大值池化（Max Pooling），并将两者拼接后经线性层压缩为固定长度的模态摘要向量 $\mathbf{u}_i \in \mathbb{R}^D$。

#### 4.2.3.4 双路径跨模态融合

本模块设计了双路径融合架构，将统计拼接与注意力交互两种互补融合策略进行集成：

**Path A（统计拼接路径）**：将四个模态摘要向量直接拼接为 $4D$ 维向量，经由多层感知机（MLP）进行跨模态关联建模：

$$\mathbf{f}_A = \text{MLP}([\mathbf{u}_V \| \mathbf{u}_A \| \mathbf{u}_K \| \mathbf{u}_{AU}])$$

该路径对模态噪声鲁棒性强，在部分模态缺失或质量退化时仍能提供稳定的融合结果。

**Path B（跨模态注意力路径）**：将四个摘要向量堆叠为序列 $[\mathbf{u}_V, \mathbf{u}_A, \mathbf{u}_K, \mathbf{u}_{AU}] \in \mathbb{R}^{4 \times D}$，输入跨模态Transformer编码器，通过模态间的交叉注意力机制捕捉高阶跨模态依赖关系：

$$\mathbf{f}_B = \text{Mean}\left(\text{TransformerEncoder}([\mathbf{u}_V, \mathbf{u}_A, \mathbf{u}_K, \mathbf{u}_{AU}])\right)$$

该路径能够显式建模视觉-音频、关键点-AU等模态之间的语义关联，有助于发现跨模态不一致缺陷。

最终融合表示为两路输出的平均值：

$$\mathbf{f} = \frac{\mathbf{f}_A + \mathbf{f}_B}{2}$$

双路径平均集成兼顾了统计稳健性与结构交互性，在模态噪声与关联捕获之间取得平衡。

#### 4.2.3.5 多任务预测头

融合表示 $\mathbf{f}$ 被并行输入五个独立的任务预测头，每个任务头由两层全连接网络（隐层维度128→64）组成，分别输出各维度的质量评分：

| 预测任务 | 评估含义 |
|---|---|
| 口型同步评分（Lip Sync） | 唇形运动与音频的时序吻合程度 |
| 表情自然度评分（Expression） | 面部表情变化的流畅性与真实感 |
| 音频质量评分（Audio Quality） | 语音清晰度、自然度与无噪声程度 |
| 跨模态一致性评分（Cross Modal） | 视听模态间的语义协调程度 |
| 综合质量评分（Overall） | 整体主观感知质量的加权汇总 |

#### 4.2.3.6 不确定性加权多任务损失

针对多任务学习中各任务损失量纲差异与梯度竞争问题，本模型采用基于预测不确定性的自适应任务加权策略（Kendall & Gal，2018）。对每个任务 $t$，模型学习一个对数方差参数 $s_t = \log \sigma_t^2$，总损失定义为：

$$\mathcal{L}_{total} = \sum_{t \in \mathcal{T}} e^{-s_t} \mathcal{L}_t + s_t$$

其中每任务基础损失为：

$$\mathcal{L}_t = \text{MSE}(\hat{y}_t, y_t) + 0.1 \cdot |\hat{y}_t - y_t|$$

当任务噪声较大时，$s_t$ 自动增大，降低该任务的有效权重，从而避免高噪声任务主导梯度更新方向。

#### 4.2.3.7 一致性正则化

为增强多任务预测之间的结构一致性，本模型引入基于任务间相关性的一致性正则化损失。设批次内各任务预测值构成矩阵 $\mathbf{P} \in \mathbb{R}^{B \times K}$（$K$为任务数），一致性损失定义为：

$$\mathcal{L}_c = 1 - \frac{2}{K(K-1)} \sum_{i < j} \text{Corr}(P_i, P_j)$$

总训练目标为：

$$\mathcal{L} = \mathcal{L}_{total} + \lambda_c \mathcal{L}_c, \quad \lambda_c = 0.05$$

该正则化项鼓励各任务预测在批次层面保持正相关，抑制单任务过拟合导致的表示坍塌。

#### 4.2.3.8 模型训练配置

模型采用Adam优化器进行参数更新，初始学习率为 $1 \times 10^{-4}$，权重衰减系数为 $1 \times 10^{-4}$，配合余弦退火学习率调度策略（CosineAnnealingLR），在训练过程中平滑降低学习率至最小值0。为在有限计算资源下提升训练效率，启用混合精度训练（AMP）与梯度累积（累积步数4），等效批次大小为32。训练数据集采用CH-SIMS情感语料库及对应标注数据，共计1602个样本，按约60%/15%/25%的比例划分为训练集953个样本、验证集239个样本、测试集410个样本。各模态原始特征经降维处理后的实际输入维度为：视觉特征163维（由ResNet-101输出的2048维视觉向量经PCA降维至163维）、音频特征768维、关键点特征1404维、AU特征17维，拼接后总特征维度2352维。
...
【插图占位：图4-6 前端主要页面交互流程图（登录 → 配置数字人 → 配置声音 → 输入文本 → 触发推理 → 查看结果）】
...
质量评估模型以独立推理服务的形式集成于后端处理管线的末端。在数字人视频生成完成后，后端服务自动调用评估模型对生成视频执行以下操作：

1. 解码视频文件，按帧提取视频帧与对应音频片段；
2. 分别调用视觉特征提取器、音频特征提取器、关键点检测器及AU检测器，生成四路特征序列；
3. 将特征序列输入已加载的多任务评估模型，执行前向推理；
4. 接收五维质量评分结果，序列化为JSON格式并写入用户任务目录；
5. 前端拉取视频时，同步返回质量评分JSON，在视频展示页面呈现各维度评分。
...
【插图占位：图4-7 系统质量评估反馈闭环示意图（生成 → 评估 → 反馈 → 参数调整 → 重生成）】
...
【插图占位：图4-9 评估模型各任务预测值与真实标注散点图（5任务，横轴为Ground Truth，纵轴为预测分）】
...
【插图占位：图4-10 主观评估平台界面截图（A/B对比评估操作界面）】
