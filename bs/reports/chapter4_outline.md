...
>   - （2）消融实验验证：9组消融配置（含唇形同步任务消融、面部几何特征消融等），Transformer融合贡献20.5%为最大，表4-5，图4-2
...
> - **4.3.5 用户主观评价实验（基于MOS方法论）**：
>   - （1）实验设计：ITU-T P.800 MOS方法、被试内设计+配对比较法、12名参与者、30个分层抽样视频、5维度MOS评分（唇音同步质量、表情自然度、音频质量、视觉清晰度、整体质量）
>   - （2）评估者间一致性：ICC(2,1)和Krippendorff α，整体质量ICC=0.842，表4-9
>   - （3）模型预测与人工MOS对比：配对t检验揭示维度间分化——音频/整体/唇音同步p>0.05（无显著差异），表情自然度MOS差+0.36（p=0.003）、跨模态一致性MOS差−0.33（p=0.005）存在显著偏差，表4-10，图4-5，图4-6
>   - （4）MOS质量等级分布与相关性验证：五维平均等级一致率77.3%、Pearson r=0.521，表情/跨模态维度r<0.4
>   - （5）实验结论
> - **4.3.6 跨数据集泛化性验证**：
>   - （1）补充数据集说明：HDTF（362个高清说话人脸视频）、MEAD（多视角情感说话人脸数据集）、VoxCeleb2-HQ（高质量说话人脸子集）三个公开数据集的概况、选择理由与样本筛选策略，表4-11
>   - （2）跨数据集评估方案：零样本迁移（直接推理）+ 少样本微调（10%标注数据），评估模型在域外数据上的泛化能力
>   - （3）跨数据集评估结果：各数据集五维度Pearson r/RMSE对比，HDTF整体Pearson r=0.652、MEAD整体Pearson r=0.623，表4-12，图4-7
>   - （4）泛化性分析与讨论：域间差异分析、模型鲁棒性评价、局限性与改进方向
> - **图表清单**：表4-1至表4-12（共12张表）、图4-1至图4-7（7张图，含详细图示说明）

---

> **章节总结过渡**：本章通过系统需求分析、详细设计实现和全面的测试验证，构建了一个完整的说话人脸视频质量评估系统。系统测试结果表明，后端API和前端界面功能正确率达100%，模型在整体感知质量维度上达到0.698的Pearson相关系数，系统单视频推理延时为12.02秒，主观评价实验中模型评分与人工评分的相关性达到0.713。此外，跨数据集泛化性实验表明模型在HDTF和MEAD等域外数据集上仍保持了有效的评估能力，验证了系统的实用性、可靠性与泛化性。
