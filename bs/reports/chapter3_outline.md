...
> - **3.2.2.1 视觉特征提取**：ResNet101提取2048维视觉特征，含帧采样、人脸检测、ImageNet归一化、全局平均池化公式、GPU分组推理与AMP优化
> - **3.2.2.2 音频特征提取**：HuBERT-Base模型提取768维音频嵌入，含FFmpeg音频分离、Wav2Vec2预处理、线性插值时间对齐公式
> - **3.2.2.3 面部关键点特征提取**：MediaPipe Face Mesh检测468个三维关键点（1404维），含坐标展平、零填充与几何拓扑信息分析
> - **3.2.2.4 面部动作单元特征提取**：基于关键点几何距离计算17个AU强度值，含AU1-AU28详细计算公式、表3-1完整定义、最大值归一化
> - **3.2.2.5 唇形同步分数计算**：SyncNet模型输出置信度与偏移量，Sigmoid映射公式将分数映射至[0,5]区间
> - **3.2.2.6 特征预处理与降维**：中位数插补公式→Z-score标准化公式→PCA降维（含协方差矩阵特征值分解公式、累计解释方差比公式），表3-2降维配置（4237→367维，降维率91.3%）
> - **3.2.2.7 GPU加速与批量处理优化**：批量推理、AMP混合精度、多线程音频加载、模型预热
> - **3.2.2.8 本节小结**
