...
> - **3.3.3 实验环境与训练配置**：（1）硬件与软件环境——RTX 4060 8GB GPU、PyTorch 2.1.0+CUDA 12.1，表3-8环境配置表；（2）训练配置——标准配置（3层/8头/256维/Adam/30轮）vs 优化配置（6层/16头/512维/AdamW/150轮），表3-9超参数对比表，固定随机种子seed=42
> - **3.3.4 对比实验与消融实验设计**：E0–E8共9组消融实验，覆盖模态特征（E1–E4）、融合架构（E5）、降维策略（E6）、训练策略（E7–E8）四个验证维度，表3-10消融实验设计方案
> - **3.3.5 主观评价实验设计**：（1）被试内A/B对比范式与单盲控制；（2）参与者招募与培训；（3）分层抽样实验材料；（4）Web端平台与质量控制（注意力检查、响应时间监控）；（5）统计分析方法（ICC、Pearson/Spearman相关、配对t检验/Wilcoxon检验）
> - **图表**：表3-5（标签统计）、表3-6（数据划分）、表3-7（评估指标体系）、表3-8（实验环境）、表3-9（超参数对比）、表3-10（消融实验设计）
