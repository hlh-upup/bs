...
算法的第三阶段负责将降维后的四类异构模态特征映射至统一的语义空间，并通过时序注意力与跨模态融合实现信息的深层交互。具体而言，四路独立的三层残差特征编码器（包含LayerNorm归一化与残差捷径连接）分别将各模态特征映射至统一的512维语义空间；共享权重的8头时序自注意力模块对各模态的编码序列进行独立的帧间时序建模；均值-最大值双池化将时序特征压缩为固定维度向量后，经由Transformer跨模态注意力路径与多尺度融合路径并行处理并取均值，最终得到512维的全局融合特征向量，供后续预测头使用。该阶段的详细设计见3.2.3节。
...
经过3.2.2节的多模态特征提取与预处理后，系统获得了四类经PCA降维的标准化时序特征矩阵。本节阐述残差特征编码、单模态时序注意力建模、跨模态双路融合以及多任务预测头的联合设计。图3-3展示了该模块的整体架构。

> **【图3-3 跨模态融合编码与多任务预测模块架构图】**
>
> 图示说明：该图应展示从降维后特征输入到质量评分输出的完整数据流。左侧为四路降维后特征输入（视觉100维、音频200维、关键点50维、AU 17维），分别经过四路并行的三层残差特征编码器（各含三级Linear+LayerNorm+ReLU+Dropout，外加残差捷径连接）映射至统一的512维语义空间。编码后的四路时序特征各自经过共享权重的时序自注意力模块（8头多头自注意力+FFN+残差+层归一化），再经均值-最大值双池化与线性投影压缩为512维向量。中部展示双路融合过程：路径一——四路向量堆叠为(4×512)矩阵后送入多层Transformer编码器（含可学习位置编码），对模态维度进行全局自注意力交互，输出经均值池化得512维向量；路径二——四路向量经多尺度融合网络（各自投影→拼接→两层全连接）得512维向量；两路输出取平均得到最终512维融合特征。右侧展示五个独立的改进型任务预测头（各含可学习特征缩放→Linear+LayerNorm+GELU+Dropout→Linear+LayerNorm+GELU→Linear→Sigmoid→\[1.0,5.0\]），分别输出五个维度的质量评分。底部标注MSE+L1混合损失与可学习Sigmoid权重、跨任务一致性正则化、以及标签掩码机制。

**（1）残差特征编码与时序注意力建模**

**① 三层残差特征编码器**

由于四类模态特征经PCA降维后的维度仍存在差异（视觉100维、音频200维、关键点50维、AU 17维），直接在特征维度上进行拼接或求和操作无法实现有效的语义对齐。为此，本研究为每类模态设计了独立的三层残差特征编码器（ImprovedFeatureEncoder），将不同维度的特征统一映射至相同的 $d_{\text{embed}}$ 维语义空间。

与简单的两层前馈网络不同，本研究采用的残差编码器包含三级逐步降维的编码层，每级均配备层归一化（LayerNorm）以稳定激活值分布。具体而言，模态 $m$ 的编码器 $\phi_m(\cdot)$ 定义为：

$$\phi_m(\mathbf{x}) = f_3(f_2(f_1(\mathbf{x}))) + g(\mathbf{x})$$

其中 $f_i$ 为第 $i$ 级编码层，$g(\cdot)$ 为残差投影（当输入维度与输出维度不一致时为线性投影 $g(\mathbf{x}) = \mathbf{W}_{\text{res}} \mathbf{x}$，否则为恒等映射）。每级编码层的结构为：

$$f_i(\mathbf{h}) = \text{Dropout}(\text{ReLU}(\text{LayerNorm}(\text{Linear}(\mathbf{h}))))$$

三级编码层依次将特征维度从输入维度 $d_m$ 逐步降至 $4d_{\text{embed}}$、$2d_{\text{embed}}$、$d_{\text{embed}}$，形成"宽→窄"的漏斗结构。以视觉模态为例（$d_m=100, d_{\text{embed}}=512$），三级编码层的维度变换为 $100 \rightarrow 2048 \rightarrow 1024 \rightarrow 512$。这种渐进式维度变换配合残差捷径连接，既保证了特征信息在深层网络中的有效传播，又避免了梯度消失问题。各级编码层的Dropout丢弃率统一设为0.3。

经过编码映射后，四类模态特征在时间维度上对齐为统一的表示形式：

$$\mathbf{E}_m = \phi_m(\tilde{\mathbf{F}}^{(m)}) \in \mathbb{R}^{T \times d_{\text{embed}}}, \quad m \in \{\text{visual}, \text{audio}, \text{keypoint}, \text{au}\}$$

**② 单模态时序自注意力建模**

在将各模态特征进行跨模态融合之前，本研究引入了一个共享权重的时序自注意力模块（TemporalAttention），分别对四类模态的编码后时序特征进行独立的时序建模。该模块的设计目的是在单模态层面先捕捉帧间的时序依赖关系，为后续的跨模态融合提供更具时序感知能力的特征表示。

时序注意力模块采用标准的Transformer编码器层结构，由多头自注意力子层和前馈神经网络子层串联构成，两个子层均配备残差连接与层归一化：

$$\mathbf{E}_m' = \text{LayerNorm}(\mathbf{E}_m + \text{Dropout}(\text{MHSA}(\mathbf{E}_m, \mathbf{E}_m, \mathbf{E}_m)))$$

$$\mathbf{E}_m'' = \text{LayerNorm}(\mathbf{E}_m' + \text{Dropout}(\text{FFN}(\mathbf{E}_m')))$$

其中多头自注意力采用8个注意力头，Dropout率为0.1，前馈维度为 $4 \times d_{\text{embed}}$。需要强调的是，四类模态共享同一个时序注意力模块的权重参数，而非为每类模态训练独立的注意力层。这一共享设计基于以下考虑：第一，四类模态在编码后均已被映射至统一的 $d_{\text{embed}}$ 维语义空间，具有相同的特征结构，因此共享注意力参数是合理的；第二，共享参数显著减少了模型的总参数量，降低了在中等规模数据集（1,985个样本）上过拟合的风险；第三，共享的注意力机制使模型能够学习到跨模态通用的时序依赖模式。

**③ 均值-最大值双池化**

经过时序注意力建模后，各模态的时序特征序列 $\mathbf{E}_m'' \in \mathbb{R}^{T \times d_{\text{embed}}}$ 需要被压缩为固定维度的向量表示。本研究采用均值池化与最大值池化相结合的双池化策略，以同时捕捉时序特征的整体统计趋势和显著峰值响应：

$$\mathbf{r}_m^{\text{avg}} = \frac{1}{T}\sum_{t=1}^{T} \mathbf{e}_{m,t}'', \quad \mathbf{r}_m^{\max} = \max_{t=1}^{T} \mathbf{e}_{m,t}''$$

$$\mathbf{r}_m = \mathbf{W}_{\text{pool}} [\mathbf{r}_m^{\text{avg}} \| \mathbf{r}_m^{\max}] + \mathbf{b}_{\text{pool}}$$

其中 $[\cdot \| \cdot]$ 表示向量拼接，$\mathbf{W}_{\text{pool}} \in \mathbb{R}^{d_{\text{embed}} \times 2d_{\text{embed}}}$ 为可学习的线性投影矩阵，将拼接后的 $2d_{\text{embed}}$ 维向量重新映射回 $d_{\text{embed}}$ 维。均值池化捕捉了整个视频时序上的平均语义信息，最大值池化则提取了最显著的特征响应。相比简单的全局平均池化，双池化策略能够保留更丰富的时序统计信息，尤其有利于捕捉瞬时的表情变化或唇形失同步等局部显著事件。

**（2）跨模态双路融合编码**

**① Transformer跨模态融合路径**

经过上述处理后，四类模态各自产生一个 $d_{\text{embed}}$ 维的紧凑向量表示 $\mathbf{r}_m$。本研究将四路向量沿新增的模态维度进行堆叠，构成多模态特征矩阵：

$$\mathbf{R} = [\mathbf{r}_{\text{visual}}, \mathbf{r}_{\text{audio}}, \mathbf{r}_{\text{keypoint}}, \mathbf{r}_{\text{au}}]^\top \in \mathbb{R}^{4 \times d_{\text{embed}}}$$

该矩阵被送入多层Transformer编码器进行跨模态注意力交互。Transformer编码器在模态维度上（而非时间维度上）执行自注意力计算，使得每个模态的表示能够根据其他模态的信息进行动态调整。编码器配备可学习的位置编码参数矩阵 $\mathbf{P} \in \mathbb{R}^{4 \times d_{\text{embed}}}$，用于标识四个模态的位置信息。

每层Transformer编码器由多头自注意力（MHSA）子层和前馈神经网络（FFN）子层构成，两个子层均采用残差连接与层归一化。多头自注意力机制的计算过程为：
...
其中 $H$ 为注意力头数，$d_k = d_{\text{embed}} / H$ 为每个注意力头的维度。前馈神经网络子层由两个线性变换和一个ReLU激活函数组成：

$$\text{FFN}(\mathbf{x}) = \text{ReLU}(\mathbf{x}\mathbf{W}_1 + \mathbf{b}_1)\mathbf{W}_2 + \mathbf{b}_2$$

其中前馈维度 $d_{\text{ff}}$ 为 $d_{\text{embed}}$ 的4倍。经过多层Transformer编码后，输出矩阵通过沿模态维度的全局平均池化压缩为 $d_{\text{embed}}$ 维的融合向量：

$$\mathbf{z}_{\text{tf}} = \frac{1}{4} \sum_{j=1}^{4} \mathbf{h}_j^{(L)} \in \mathbb{R}^{d_{\text{embed}}}$$

本研究提供了两套Transformer编码器配置，具体参数对比如表3-3所示。
...
优化配置通过增加编码层数和注意力头数，显著增强了模型对跨模态依赖关系的建模能力。

**② 多尺度融合路径**

与Transformer融合路径并行，本研究设计了一条多尺度融合路径（MultiScaleFusion），对四路模态向量进行浅层但更直接的特征组合。该路径首先将四路 $d_{\text{embed}}$ 维向量各自通过独立的线性投影层映射至统一维度，然后在特征维度上进行拼接，再经过两层全连接网络融合为 $d_{\text{embed}}$ 维向量：

$$\mathbf{z}_{\text{ms}} = \text{FC}_2(\text{ReLU}(\text{FC}_1([\mathbf{W}_1'\mathbf{r}_{\text{visual}} \| \mathbf{W}_2'\mathbf{r}_{\text{audio}} \| \mathbf{W}_3'\mathbf{r}_{\text{keypoint}} \| \mathbf{W}_4'\mathbf{r}_{\text{au}}])))$$

其中 $\text{FC}_1 \in \mathbb{R}^{4d_{\text{embed}} \rightarrow 2d_{\text{embed}}}$，$\text{FC}_2 \in \mathbb{R}^{2d_{\text{embed}} \rightarrow d_{\text{embed}}}$，中间配备Dropout（丢弃率0.2）以防止过拟合。

**③ 双路融合输出**

最终，两条融合路径的输出取均值作为全局融合特征向量：

$$\mathbf{z} = \frac{1}{2}(\mathbf{z}_{\text{tf}} + \mathbf{z}_{\text{ms}}) \in \mathbb{R}^{d_{\text{embed}}}$$

这种双路融合设计兼顾了深层注意力交互（Transformer路径）与浅层直接特征组合（多尺度路径）的互补优势：Transformer路径能够通过全局自注意力机制动态学习模态间的复杂依赖关系，但其计算复杂度较高且可能在训练初期出现注意力分散的问题；多尺度融合路径通过投影-拼接-全连接的简单结构，提供了一条稳定的梯度回传通道，有助于训练初期的快速收敛。两路取均值融合使得最终特征同时包含深层交互语义和浅层组合信息，提升了特征表示的鲁棒性。

**（3）多任务预测头与动态权重策略**

**① 改进型任务预测头**

基于双路融合后的 $d_{\text{embed}}$ 维全局特征向量 $\mathbf{z}$，本研究为五个评估维度分别设计了独立的改进型任务预测头（ImprovedTaskHead）。相比简单的全连接预测头，改进型预测头引入了以下增强设计：

第一，**可学习的特征缩放参数**。每个预测头包含一个标量参数 $\alpha_k$（初始化为1.0），在特征进入预测网络前对其进行缩放：$\tilde{\mathbf{z}}_k = \alpha_k \cdot \mathbf{z}$。该设计使各预测头能够自适应地调整输入特征的幅度，适应各评估维度不同的敏感度需求。

第二，**层归一化与GELU激活**。各隐藏层采用LayerNorm进行归一化，配合GELU（Gaussian Error Linear Unit）激活函数替代传统ReLU。GELU函数 $\text{GELU}(x) = x \cdot \Phi(x)$（其中 $\Phi(x)$ 为标准正态分布的累积分布函数）相比ReLU具有更平滑的梯度特性，有助于模型收敛。

第三，**差异化Dropout策略**。隐藏层的Dropout率设为0.3，而最后一层隐藏层的Dropout率减半（0.15），避免在接近输出端的层中过度丢弃信息。

第 $k$ 个任务预测头的完整结构为：

$$\text{Linear}(d_{\text{embed}}, 256) \rightarrow \text{LayerNorm} \rightarrow \text{GELU} \rightarrow \text{Dropout}(0.3) \rightarrow \text{Linear}(256, 128) \rightarrow \text{LayerNorm} \rightarrow \text{GELU} \rightarrow \text{Dropout}(0.15) \rightarrow \text{Linear}(128, 1) \rightarrow \text{Sigmoid}$$

Sigmoid激活函数将网络输出约束在 $[0, 1]$ 范围内，再通过仿射变换映射至 $[1.0, 5.0]$ 评分区间：$\hat{y}_k = 1.0 + 4.0 \cdot \sigma(\cdot)$，并进一步通过Clamp操作确保预测值严格位于 $[1.0, 5.0]$ 范围内。各预测头之间共享残差编码器、时序注意力和融合模块的参数，但拥有独立的预测层参数，这种设计既利用了共享特征表示中的跨任务互补信息，又通过独立参数适应了各任务的特异性需求。

**② 混合损失函数与标签掩码**

本研究采用MSE（均方误差）与L1（平均绝对误差）混合损失作为单任务基础损失函数，以同时兼顾对大误差的惩罚力度和对小误差的稳健性。单任务损失定义为：

$$\mathcal{L}_k^{\text{base}} = \text{MSE}(\hat{y}_k, y_k) + 0.1 \cdot \text{L1}(\hat{y}_k, y_k)$$

其中L1损失项的系数0.1为经验超参数，使MSE损失占主导地位的同时，L1项为预测偏差分布提供了更稳健的梯度信号。

由于数据集中部分维度的标签存在缺失（部分视频未标注所有五个维度的分数），直接对所有样本计算损失会引入无效梯度信号。为此，本研究设计了基于有效标签掩码的损失计算机制。对于第 $k$ 个任务，定义有效标签掩码 $\mathbf{m}_k \in \{0, 1\}^N$，其中 $m_{k,i} = 1$ 表示第 $i$ 个样本具有任务 $k$ 的有效标注，无效标签（-1.0）通过掩码排除于损失计算之外。

在此基础上，各任务损失通过可学习的Sigmoid权重进行动态加权。本研究为每个任务定义一个可学习的标量参数 $\omega_k$（初始化为0），通过Sigmoid函数将其映射至 $(0, 1)$ 区间作为有效权重：

$$w_k = \sigma(\omega_k), \quad \mathcal{L}_k = w_k \cdot \mathcal{L}_k^{\text{base}}$$

这种设计使各任务的损失权重能够在训练过程中自动调整：当某个任务的学习已趋于饱和时，其对应的 $\omega_k$ 可以自然降低以减少对该任务的优化投入；反之，仍需优化的任务会获得更高的有效权重。

**③ 跨任务一致性正则化**

为鼓励各子维度的预测评分保持合理的内在一致性，本研究引入了跨任务一致性正则化损失。该正则项计算四个子维度预测值（不含整体感知质量维度）的跨任务标准差均值：

$$\mathcal{L}_{\text{cons}} = \frac{1}{N} \sum_{i=1}^{N} \text{std}([\hat{y}_{\text{lip\_sync},i}, \hat{y}_{\text{expression},i}, \hat{y}_{\text{audio},i}, \hat{y}_{\text{cross\_modal},i}])$$

该正则项以权重系数0.1加入总损失中，作为软约束引导各子维度的预测分布趋于协调。

多任务联合损失定义为：

$$\mathcal{L}_{\text{total}} = \sum_{k=1}^{K} w_k \cdot \mathcal{L}_k^{\text{base}} + 0.1 \cdot \mathcal{L}_{\text{cons}}$$

其中 $K=5$ 为任务总数，$w_k = \sigma(\omega_k)$ 为可学习的Sigmoid权重。

**④ 动态任务权重策略（高级模型扩展）**

除上述基于可学习Sigmoid参数的自适应权重方案外，本研究在高级模型变体（AdvancedMultiTaskTalkingFaceEvaluator）中还实现并对比了以下三种更复杂的动态任务权重策略，以进一步探索多任务权重均衡的最优方案。
...
以上设计构成了一条完整的"三层残差编码→时序自注意力→均值-最大值双池化→跨模态双路融合→改进型任务预测头"处理管线。残差编码器通过三级渐进降维与残差捷径连接实现了稳健的跨模态特征映射；共享权重的时序注意力模块在单模态层面捕捉帧间依赖关系；双路融合机制兼顾了深层注意力交互与浅层特征组合的互补优势；改进型预测头引入可学习特征缩放、LayerNorm、GELU激活和差异化Dropout提升了预测精度。训练端采用MSE+L1混合损失、可学习Sigmoid权重、跨任务一致性正则化和标签掩码机制，保障了多任务训练的稳定性与均衡性。
