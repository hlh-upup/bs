...
此外，数据集中部分维度存在标签缺失的情况，需要在损失计算中加以处理。统计分析表明，唇形同步维度的标签完整率为100%，表情自然度的有效标签比例为72.8%，音频质量为77.8%，跨模态一致性和整体感知质量均为72.6%，其余样本的标签以无效值（-1.0）标记。这意味着不同任务在每个训练批次中的有效样本数量可能存在差异，需要通过掩码机制妥善处理。因此，如何设计自适应的动态任务权重策略，使模型能够在多任务联合优化中有效缓解梯度冲突、协调各任务的训练进度，是本章面临的第二个问题。
...
|---------|:---------:|:----------------:|:------:|:----:|:-----:|:-----:|:-----:|
| 唇形同步（lip\_sync） | 1,985 | 0 | 100.0% | 3.58 | 0.53 | 1.0 | 5.0 |
...
从表3-5可以观察到以下数据特征：第一，五个评估维度的标签分布总体均衡，各维度均值分布在3.18–3.58之间，标准差在0.52–0.61之间，评分覆盖了1.0–5.0的完整区间，表明数据集中的视频样本在各质量维度上均具有良好的区分度；第二，表情自然度、跨模态一致性和整体感知质量三个维度存在部分标签缺失（标记为-1.0），本研究通过标签掩码机制在训练时排除这些无效样本，确保损失计算的准确性。
...
第四，唇形同步（lip\_sync）维度的Pearson $r$为0.554，$R^2$为0.287。唇形同步质量的评估需要精确捕捉口型运动与语音信号之间的时序对齐关系，对模型的时序建模能力要求较高。面部关键点特征中口唇区域关键点的运动模式为该维度的预测提供了显式的几何线索，同时SyncNet提供的唇形同步分数作为辅助参考信息，在一定程度上补充了该维度的评估信息。
