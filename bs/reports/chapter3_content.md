...
**创新点二：基于面部关键点几何距离的无外部依赖动作单元近似估计方法。** 传统面部动作单元提取依赖于OpenFace等外部工具链，存在部署复杂、模型体积大且推理效率低等问题。本研究提出了一种仅基于MediaPipe Face Mesh关键点的几何距离计算方案，通过17个关键点对之间的距离与逆距离关系近似估计核心AU的激活强度，在无需额外预训练模型的条件下实现了轻量化的面部动作编码。

**创新点三：面向多任务联合优化的不确定性自适应动态权重策略。** 在多任务联合回归中，不同评估维度的最优梯度方向可能存在冲突，固定权重策略难以动态适应各任务训练进度的差异。本研究将基于同方差不确定性的自适应权重机制与标签掩码机制相结合，通过可学习的不确定性参数自动调节各任务的损失权重，使模型在训练过程中能够根据各任务的优化状态动态分配优化资源，有效缓解了多任务学习中的梯度冲突与负迁移问题。
...
其中 $M=4$ 为模态数量。与现有多模态融合方法相比，本研究选择"嵌入对齐+均值融合+时序Transformer"的级联策略，而非直接在原始异构特征上应用cross-attention。这一设计基于以下考虑：第一，四类模态特征的原始维度差异悬殊（从17维到2048维），若直接应用cross-attention需要处理非对称的Query-Key-Value维度映射，计算复杂度高且训练不稳定；第二，通过模态嵌入层的显式对齐，各模态被映射至同一语义空间后再进行均值融合，既保持了各模态的等权贡献，又使后续Transformer编码器可以在统一的语义尺度上建模时序依赖关系。

由于融合后的每个时间步向量均包含了四类模态的混合语义，Transformer编码器的自注意力机制在时间步维度上的全局建模实际上隐式地捕捉了多模态信息的跨时间步联合演化规律。
...
综合而言，本章的核心创新贡献体现在以下三个方面：（1）提出了面向说话人脸视频的多模态多任务联合质量评估框架，首次实现了唇形同步、表情自然度、音频质量、跨模态一致性和整体感知质量五个维度的端到端统一预测，突破了现有方法仅能进行单一维度评估的局限；（2）设计了基于面部关键点几何距离的轻量化动作单元近似估计方法，在无需引入额外外部工具的条件下实现了17个核心AU的有效提取；（3）提出了基于同方差不确定性的自适应动态权重策略，通过可学习的不确定性参数自动调节各任务的损失权重，有效缓解了多任务联合优化中的梯度冲突与负迁移问题。
