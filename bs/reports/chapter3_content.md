...
本研究招募12名参与者（6名男性、6名女性，年龄20–35岁），采用被试内设计（Within-Subject Design）对30个分层抽样的视频样本进行五维度主观评价。评价维度涵盖唇音同步质量、表情自然度、音频质量、视觉清晰度（对应模型的跨模态一致性维度）和整体质量，评分采用5分李克特量表（1分=很差，5分=优秀）。实验通过Web端评价平台进行，参与者在标准化环境中佩戴监听耳机、使用24英寸IPS显示器完成评价。经注意力检查筛选后，最终保留11名参与者的有效数据。以11名参与者评分的均值作为主观评价的金标准。
...
第一，主观实验中的Pearson相关系数在所有五个维度上均略高于客观评估结果（平均高出约2.8\%），这主要是因为主观实验以11名评估者的均值作为金标准，有效平滑了个体标注噪声，提供了更稳定的参照标准。相比之下，客观标签来源于单一映射规则（情感标注线性映射或SyncNet自动提取），标签噪声相对较高。
