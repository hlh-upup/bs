...
基于3.1节的问题分析，本节给出多模态多任务说话人脸视频质量评估算法的总体设计方案。该算法以端到端的计算管线形式组织，依次经过多模态特征提取、特征预处理与降维、多模态融合Transformer时序编码以及多任务质量预测四个阶段，最终输出唇形同步、表情自然度、音频质量、跨模态一致性和整体感知质量五个维度的评分。图3-1展示了算法的总体架构。

> **【图3-1 多模态多任务说话人脸视频质量评估算法总体架构图】**
>
> 图示说明：该图应从左至右展示四个核心阶段的数据流。左侧为输入端（原始视频），依次经过：（1）多模态特征提取阶段——四路并行的特征提取器（ResNet101→2048维、HuBERT→768维、MediaPipe→1404维、AU计算→17维），总维度4237；（2）特征预处理与降维阶段——NaN插补→Z-score标准化→PCA降维，总维度压缩至367维；（3）多模态融合Transformer时序编码阶段——四路嵌入网络→模态嵌入→均值融合→可学习位置编码→6层16头Transformer编码器→全局平均池化→512维融合向量；（4）多任务质量预测阶段——五个独立预测头分别输出lip\_sync、expression、audio\_quality、cross\_modal、overall五个维度的\[1.0, 5.0\]评分。右侧标注动态任务权重与标签掩码机制。

**（1）多模态特征提取阶段**：从原始视频中并行提取视觉（ResNet101, 2048维）、音频（HuBERT, 768维）、面部关键点（MediaPipe, 1404维）和辅助几何表情编码（基于关键点距离, 17维）四类帧级时序特征，总维度为4237维。此外，SyncNet模块计算音视频同步置信度作为辅助参考信息。

**（2）特征预处理与降维阶段**：依次执行缺失值中位数插补、Z-score标准化和PCA降维，将总维度从4237维压缩至367维（降维率91.3%），各步骤的具体实现详见3.2.2节。

**（3）多模态融合Transformer时序编码阶段**：四路模态嵌入网络将各模态特征映射至统一的512维语义空间，叠加可学习的模态嵌入标识后经均值融合生成多模态混合时序序列，再经可学习位置编码后送入6层16头Transformer编码器进行深层时序编码，输出经全局平均池化聚合为512维的融合特征向量。该阶段的详细设计见3.2.3节。

**（4）多任务质量预测阶段**：基于融合特征向量，五个独立的三层预测头分别输出$[1.0, 5.0]$评分区间的质量分数。训练阶段采用动态任务权重与标签掩码机制协调多任务优化，具体策略见3.2.3节。

综合来看，本算法具有模块化架构（各阶段功能职责明确、接口清晰）和端到端可训练（从特征嵌入层到多任务预测头均可通过反向传播联合优化）两大特点。本研究最终选用优化配置（编码维度512、6层Transformer、16注意力头）作为正式模型架构，同时保留标准配置（编码维度256、3层Transformer、8注意力头）作为消融实验的对比基线，两套配置通过统一的YAML文件切换。上述设计方案的各模块将在3.2.2和3.2.3小节中展开阐述。

### 3.2.2 多模态特征提取模块

多模态特征提取是整个评估算法的基础环节，本节阐述四类模态特征的提取方法、唇形同步分数的计算方式以及特征预处理与降维流程。图3-2展示了该模块的整体架构。

> **【图3-2 多模态特征提取模块架构图】**
>
> 图示说明：该图应以输入视频为起点，展示五路并行的特征提取流程：（1）视觉路径——视频帧→RetinaFace人脸检测→224×224裁剪→ImageNet归一化→ResNet101（去FC层）→GAP→2048维/帧；（2）音频路径——FFmpeg分离音频→16kHz PCM→HuBERT-Base→768维→线性插值对齐至150帧；（3）关键点路径——MediaPipe Face Mesh→468个3D关键点→展平为1404维/帧；（4）辅助表情编码路径——基于关键点几何距离→17维表情编码特征→最大值归一化；（5）SyncNet路径——音视频同步置信度与偏移量→Sigmoid映射→\[0,5\]分数。下方展示预处理流程：NaN中位数插补→Z-score标准化→PCA降维（表3-2配置），最终输出367维标准化特征。
...
给定一个输入视频 $V$，系统首先将其统一采样为 $T=150$ 帧（目标帧率 $f_{\text{target}}=25$ fps）的标准序列。对于第 $t$ 帧图像 $I_t$（$t=1,2,\ldots,T$），系统通过RetinaFace人脸检测器定位面部区域，将裁剪后的人脸图像缩放至 $224\times224$ 像素并按ImageNet标准进行通道归一化。归一化后的帧图像被送入ResNet101网络，移除最后的全连接分类层，以全局平均池化层的输出作为视觉特征向量：

$$\mathbf{v}_t = \text{GAP}(\text{ResNet101}(\hat{I}_t)) \in \mathbb{R}^{2048}$$

最终，每个视频样本的视觉特征矩阵为 $\mathbf{V} = [\mathbf{v}_1, \mathbf{v}_2, \ldots, \mathbf{v}_T]^\top \in \mathbb{R}^{150 \times 2048}$。

**② 音频特征提取**

音频特征负责表征语音信号的清晰度、自然度和韵律特征。本研究采用Meta AI提出的HuBERT（Hidden-Unit BERT）自监督预训练语音模型作为音频特征编码器。

音频特征的提取流程如下：首先通过FFmpeg工具从输入视频中分离音频流，转换为采样率 $f_s = 16000$ Hz、单声道的PCM格式波形信号 $\mathbf{a}$。波形信号经Wav2Vec2特征提取器预处理后，送入HuBERT-Base模型（包含7层卷积编码器和12层Transformer编码器），提取最后一层隐藏状态作为768维的音频特征。由于HuBERT卷积编码器对输入波形进行了约320倍降采样（每20ms一帧），输出时间步数 $T_a$ 与视频帧数 $T=150$ 通常不一致，本研究采用线性插值将音频特征统一调整为150个时间步：

$$\mathbf{A} = \text{Interpolate}(\text{HuBERT}(\mathbf{a}), T) \in \mathbb{R}^{150 \times 768}$$

**（2）面部关键点与辅助几何表情编码特征提取**
...
为从面部关键点中提取更紧凑的表情动态描述，本研究基于MediaPipe Face Mesh输出的468个面部关键点，通过特定关键点对之间的欧氏距离与逆距离关系，计算17维辅助几何表情编码特征。该特征借鉴了面部动作编码系统（FACS）中动作单元（Action Unit, AU）的分区思想，分别编码了眉部（如内侧/外侧眉毛提升）、眼部（如眼睑开合）、口部（如双唇分开、下颌张开）和鼻部（如鼻翼运动）等区域的几何变化信息。

需要明确的是，该方案的定位为**辅助几何表情编码**而非精确的动作单元识别。基于关键点距离的近似估计在精度上不及专用AU识别模型（如OpenFace），但在本研究的质量评估框架中，该特征主要作为面部动态信息的低维辅助补充，用于捕捉关键点坐标所不易直接反映的局部运动模式（如眼睑收紧、嘴唇内卷等通过逆距离关系编码的收缩性运动）。该方案无需引入额外的预训练模型，降低了系统部署的复杂度。消融实验（3.4.2节E4）表明，该特征的贡献虽有限（−4.0%）但稳定，在面部微表情特征补充方面发挥了一致性作用。

具体而言，对于第 $t$ 帧的关键点集合 $\mathbf{P}_t$，各维度的强度值通过特定关键点对之间的欧氏距离 $d(\cdot, \cdot)$ 计算获得。距离型特征直接反映面部局部区域的张开程度（如眼睑开度、唇部张开度），逆距离型特征则编码收缩性运动（如眼睑收紧 $= 1/(\text{眼睑开度} + \epsilon)$）。表3-1汇总了17维特征的编码规则。
...
提取完成后，17维特征向量经过最大值归一化处理，将各维度映射至 $[0, 1]$ 区间：

$$\hat{u}_{t,j} = \frac{u_{t,j}}{\max_{t' \in \text{Train}} u_{t',j} + \epsilon}$$

其中归一化参数仅在训练集上统计获得，$\epsilon = 10^{-6}$。最终，辅助几何表情编码特征矩阵为 $\mathbf{U} = [\hat{\mathbf{u}}_1, \hat{\mathbf{u}}_2, \ldots, \hat{\mathbf{u}}_T]^\top \in \mathbb{R}^{150 \times 17}$。
...
经过上述多模态特征提取后，每个视频样本被表示为四组时序特征矩阵：视觉特征 $\mathbf{V} \in \mathbb{R}^{150 \times 2048}$、音频特征 $\mathbf{A} \in \mathbb{R}^{150 \times 768}$、关键点特征 $\mathbf{K} \in \mathbb{R}^{150 \times 1404}$ 和辅助几何表情编码特征 $\mathbf{U} \in \mathbb{R}^{150 \times 17}$。为解决3.1节分析的尺度失衡与高维冗余问题，需要在送入融合编码器之前进行预处理与降维。
...
经过上述处理，每个视频样本被表示为四组经PCA降维后的标准化时序特征（总维度367维），供3.2.3节的多模态融合编码模块使用。

### 3.2.3 多模态融合Transformer时序编码与多任务预测

经过3.2.2节的特征提取与预处理后，系统获得了四类经PCA降维的标准化时序特征。本节阐述多模态融合Transformer编码器与多任务预测头的设计。图3-3展示了该模块的整体架构。

> **【图3-3 多模态融合Transformer时序编码与多任务预测模块架构图】**
>
> 图示说明：该图应展示从降维后特征输入到质量评分输出的完整数据流。左侧为四路降维后特征输入（视觉100维、音频200维、关键点50维、辅助表情编码17维），分别经过四路并行嵌入网络（各含两层全连接+ReLU+Dropout）映射至统一的512维语义空间，并加入可学习的模态嵌入向量标识各模态来源。中部展示融合过程：四路嵌入特征经均值融合→叠加可学习位置编码→送入6层Transformer编码器（每层含16头多头自注意力+FFN+残差连接+层归一化），编码器输出经全局平均池化得到512维融合向量。右侧展示五个独立的任务预测头（512→256→128→1→Sigmoid→\[1.0,5.0\]），分别输出五个维度的质量评分。底部标注动态权重策略（固定权重/不确定性加权/GradNorm）和标签掩码机制。
...
- 辅助表情编码分支：$\text{Linear}(d_{\text{au}}, 128) \rightarrow \text{ReLU} \rightarrow \text{Dropout}(0.3) \rightarrow \text{Linear}(128, d_{\text{embed}})$

其中，视觉与辅助表情编码分支采用较小的隐藏维度（分别为256和128），是因为视觉特征经PCA降维后维度已大幅缩减，而辅助表情编码特征原始维度仅为17维；音频与关键点分支采用较大的隐藏维度（512），以保留更丰富的语义信息。
...
为使后续Transformer编码器能够感知各时间步向量中不同模态的语义来源，本研究为四类特征分别引入了可学习的模态嵌入（Modality Embedding）。具体而言，为每类模态定义一个可学习的模态偏置向量 $\mathbf{b}_m \in \mathbb{R}^{d_{\text{embed}}}$（$m \in \{\text{visual}, \text{audio}, \text{keypoint}, \text{au}\}$），在嵌入映射之后、多模态融合之前，将其加到对应的嵌入特征上：

$$\hat{\mathbf{E}}_m = \mathbf{E}_m + \mathbf{b}_m, \quad m \in \{\text{visual}, \text{audio}, \text{keypoint}, \text{au}\}$$

该设计类似于BERT中的Segment Embedding机制，通过为不同来源的特征注入显式的模态身份信息，使融合后的表示保留模态来源标识。

在此基础上，本研究将加入模态嵌入后的四组特征通过均值融合生成统一的多模态混合时序序列：

$$\mathbf{E}_{\text{fused}} = \frac{1}{M} \sum_{m=1}^{M} \hat{\mathbf{E}}_m \in \mathbb{R}^{T \times d_{\text{embed}}}$$

其中 $M=4$ 为模态数量。

**关于融合策略的设计考量。** 需要明确的是，本研究的融合架构采用"嵌入对齐+模态嵌入+均值融合+时序Transformer"的级联策略，而非在模态维度上应用cross-attention的典型跨模态Transformer架构。Transformer编码器在本框架中的核心作用是**对多模态混合时序序列进行深层时序建模**，而非在独立的模态token之间计算跨模态注意力。这一设计选择基于以下技术考量：

第一，**维度不对称性约束**。四类模态特征的原始维度差异悬殊（从17维到2048维），若直接采用cross-attention需要处理非对称的Query-Key-Value维度映射，计算复杂度为$O(M^2 \cdot T^2 \cdot d)$，在四模态150时间步的设置下训练不稳定；相比之下，先融合再做时序自注意力的计算复杂度为$O(T^2 \cdot d)$，降低了$M^2$倍。

第二，**隐式跨模态交互的有效性**。均值融合虽然在融合步骤本身是线性操作，但融合后每个时间步的向量均包含了四类模态的混合语义，且各模态成分通过模态嵌入保留了来源标识。后续Transformer编码器的自注意力机制在时间步维度上建模全局依赖关系时，实际上在不同时间步的混合模态表示之间建立了交互——即$t_1$时刻的视觉-音频混合信号与$t_2$时刻的视觉-音频混合信号之间的注意力权重，隐式地编码了跨模态信息在时间维度上的联合演化规律。消融实验（E5）表明，该架构相比特征拼接+MLP带来了20.5%的性能提升，验证了这种隐式跨模态时序建模的有效性。

第三，**任务特性的适配性**。说话人脸视频质量评估的核心挑战在于捕捉音视频信号的时序协调模式（如口型运动与语音的同步节奏、表情变化与语调的时序配合），这本质上是一个时序建模问题。将多模态信息融合为统一的时序表示后通过Transformer进行深层时序编码，恰好符合这一任务需求。
...
其中 $\mathbf{P}$ 为可学习的位置编码参数矩阵，$T=150$ 为最大序列长度。综合来看，模态嵌入提供模态身份信息，位置编码提供时序位置信息，二者共同确保Transformer编码器在处理融合后的时序序列时，既能区分不同模态的语义来源，又能感知时间步的先后顺序。
...
其中 $H$ 为注意力头数，$d_k = d_{\text{embed}} / H$ 为每个注意力头的维度，$\mathbf{W}_i^Q, \mathbf{W}_i^K, \mathbf{W}_i^V \in \mathbb{R}^{d_{\text{embed}} \times d_k}$ 和 $\mathbf{W}^O \in \mathbb{R}^{d_{\text{embed}} \times d_{\text{embed}}}$ 为可学习的投影矩阵。多头自注意力机制使得模型能够同时关注不同时间步之间的多种类型的依赖关系，在多模态混合表示的时序维度上捕捉复杂的联合演化模式。
...
优化配置通过增加编码层数和注意力头数，显著增强了模型对多模态混合时序序列中长程依赖关系的建模能力，尤其适用于需要精细捕捉音视频协调模式的评估任务。
...
如3.1.2节所分析，五个评估任务在多任务联合优化过程中面临梯度冲突与训练进度不一致的挑战，固定等权重策略难以适应各任务状态的动态变化。为解决这一问题，本研究设计并对比了以下三种动态任务权重策略。
...
在训练过程中，高损失（即高不确定性）的任务其 $\sigma_k$ 会增大，从而降低该任务的有效权重，避免困难任务对整体优化方向的过度干扰；反之，低损失任务的权重会自动提升。不确定性加权策略能够在训练过程中动态感知各任务优化难度和收敛速度的差异，自动平衡各任务的梯度贡献，有效缓解多任务联合优化中的梯度冲突与负迁移问题。
...
以上设计中，多模态融合Transformer编码器通过嵌入对齐、模态嵌入标识与均值融合将异构特征统一为多模态混合时序序列，再通过多头自注意力机制进行深层时序编码获得全局融合特征。五个独立预测头在共享表示基础上输出各维度评分，动态权重策略与标签掩码机制保障了多任务训练的稳定性与均衡性。
...
| E4 | 去除辅助表情编码（w/o AU） | 移除辅助几何表情编码特征 | 辅助表情编码特征的贡献 |
| E5 | 去除Transformer融合（w/o Transformer） | 以直接拼接+MLP替代Transformer | Transformer时序编码的贡献 |
...
**① 模态特征贡献验证**（E1–E4）：通过逐一移除各模态特征输入，量化视觉、音频、面部关键点和辅助几何表情编码四类模态特征对评估性能的独立贡献比例。其中，对于单模态去除实验，被移除模态对应的嵌入层输出以零向量替代，其余三路模态正常参与融合编码。

**② 融合架构贡献验证**（E5）：将Transformer时序编码器替换为简单的特征拼接+两层MLP结构，验证基于注意力机制的深层多模态时序建模相比浅层特征拼接的性能优势。
...
第三，跨模态一致性（cross\_modal）维度的Pearson $r$为0.596，CCC为0.543。该维度的评估依赖于模型对多模态信号间时序对齐与语义一致性的联合建模能力，Transformer的多头自注意力机制通过在多模态混合时序序列上建立全局依赖关系，有效捕捉了不同时间步之间的音视频协调模式，为该维度的评估提供了架构基础。
...
四类模态特征对模型性能的贡献存在明显差异。音频特征的去除导致了最大的性能下降（−14.5%），表明音频模态在该任务中具有关键作用——音频信号直接关系到语音清晰度与唇音同步度，是多个评估维度的主要信息来源。视觉特征的去除造成10.0\%的性能下降，说明ResNet101提取的视觉表征为面部图像质量与整体感知评估提供了重要的外观信息。关键点特征的贡献为6.9\%，反映了面部几何结构信息在表情自然度和跨模态一致性评估中的辅助价值。辅助几何表情编码特征的贡献有限但稳定（−4.0\%），与其作为低维辅助补充特征的定位一致——该特征维度仅为17维且基于关键点距离的近似估计，信息容量有限，但在多次重复实验中均表现出一致性的性能贡献，表明其所编码的面部局部运动模式（如眼睑收紧、唇部开合等）为质量评估提供了稳定的辅助信号。

综合来看，四类模态特征的贡献排序为：音频（14.5\%）> 视觉（10.0\%）> 关键点（6.9\%）> 辅助表情编码（4.0\%），验证了多模态特征融合的必要性——任一模态的缺失均会导致性能退化。

**（2）融合架构贡献分析**（E5）

Transformer多模态融合时序编码器是对整体性能影响最大的单一模块，去除后平均Pearson $r$从0.581下降至0.462，降幅达20.5\%。替换为特征拼接+MLP结构后，模型无法通过全局自注意力机制建立不同时间步上多模态混合信号之间的深层依赖关系，只能进行浅层特征组合。这一结果表明，基于注意力机制的多模态时序建模能够有效捕捉音视频信号的时序协调模式，通过动态计算不同时间步之间的注意力权重实现信息的选择性聚合，是多模态融合质量评估中的关键环节。
...
在模型架构方面，优化配置（E0：6层/16头/512维）相比标准配置（E8：3层/8头/256维）提升了8.1\%（从0.534提升至0.581）。更深的Transformer编码器和更宽的隐藏维度赋予了模型更强的多模态时序特征交互能力与非线性拟合能力，因此本研究最终选择优化配置作为模型的最优架构方案。
...
本章围绕说话人脸视频质量评估任务，提出并验证了一种基于多模态融合Transformer的多任务质量评估算法。

在算法设计层面，本章构建了"多模态特征提取→特征预处理与PCA降维→多模态融合Transformer时序编码→多任务质量预测"的端到端处理管线，通过PCA将原始4,237维特征压缩至367维，经模态嵌入对齐与均值融合生成多模态混合时序序列后，由Transformer编码器进行深层时序建模，并借助不确定性加权策略实现五维质量评分的联合预测。

在实验验证层面，基于EmotionTalk数据集（1,985个样本）的实验结果表明，模型在五个评估维度上的平均Pearson相关系数为0.581，其中整体感知质量维度最优（$r$=0.698，CCC=0.651）。消融实验证实了Transformer多模态融合时序编码器（贡献20.5%）、音频特征（14.5%）和PCA降维（11.9%）是性能提升的三个主要因素。

本章的核心创新贡献体现在两个方面：（1）首次构建了面向说话人脸视频的五维联合质量评估框架，涵盖唇形同步、表情自然度、音频质量、跨模态一致性和整体感知质量五个评估维度，其中跨模态一致性维度的引入使得评估框架能够综合衡量多模态信号间的协调一致程度，是区别于传统单维度评估方法的核心设计；（2）提出了基于同方差不确定性的自适应动态权重策略，有效缓解了多任务联合优化中的梯度冲突与负迁移问题。本章的算法设计与实验验证为第4章的系统集成与部署提供了技术基础。
