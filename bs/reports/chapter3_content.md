...
然而，对EmotionTalk数据集中五个维度标签分布的深入统计分析揭示了一个严重的标签不均衡问题。在全部1,985个视频样本中，唇形同步维度的标签分布极为集中（均值为4.763，标准差仅0.025），评分几乎全部落在4.6–4.9的窄区间内，呈现出极端的低方差特性；与之形成鲜明对比的是，表情自然度（标准差约0.590）、音频质量（标准差约0.519）、跨模态一致性（标准差约0.566）和整体感知质量等其余维度的标签则呈现出正常的分布特征和合理的方差水平。这一现象表明，数据集中的说话人脸视频在唇形同步质量上高度一致（普遍达到较高水平），但在表情表达、音频品质和跨模态协调等维度上存在显著的质量差异。

上述标签分布的严重不均衡给多任务学习框架的设计带来了本质性的挑战。在标准的多任务学习范式中，总损失函数通常表示为各子任务损失的加权和：

$$L_{total} = \sum_{k=1}^{K} w_k \cdot L_k$$

其中 $K=5$ 为任务数量，$w_k$ 为第 $k$ 个任务的权重，$L_k$ 为对应的均方误差损失。当采用固定等权重策略（$w_k=1, \forall k$）时，由于唇形同步任务的标签方差极低（标准差仅0.025，约为其他维度的1/20），其损失值量级远小于其他任务，导致该任务在总损失中的梯度贡献几乎可以忽略，模型将无法学习到有意义的唇形同步评估能力。反之，若为唇形同步任务设置过大的权重以补偿其低方差特性，则该任务的梯度信号可能反过来干扰其他任务的正常优化过程，引发负迁移（negative transfer）现象。
...
**创新点三：面向极端标签不均衡的不确定性自适应多任务权重策略。** 针对数据集中唇形同步维度标签方差极低（标准差仅0.025，约为其他维度的1/20）且存在27\%–28\%标签缺失的极端不均衡场景，本研究将基于同方差不确定性的自适应权重机制与标签掩码机制相结合，使模型能够在标签分布极端不均衡且部分缺失的条件下自动调节各任务的优化力度，避免低方差任务的梯度信号被湮没或反过来干扰其他任务。
...
如3.1.2节所分析，五个评估任务之间存在严重的标签分布不均衡现象。特别是lip\_sync任务的标签方差极低（均值4.763，标准差仅0.025），若采用等权策略，该任务的梯度信号将被其他高方差任务严重淹没，导致训练过程中该任务难以有效学习。为解决这一问题，本研究设计并对比了以下三种动态任务权重策略。
...
在训练过程中，高损失（即高不确定性）的任务其 $\sigma_k$ 会增大，从而降低该任务的有效权重，避免困难任务对整体优化方向的过度干扰；反之，低损失任务的权重会自动提升，使模型将更多优化资源分配给已具备较好基础的任务。值得强调的是，不确定性加权策略在本研究的应用场景中具有特殊的适配价值：当lip\_sync任务的标签方差极低（标准差仅0.025）时，该任务的MSE损失值天然很小，对应的 $\sigma_k$ 会收敛至较小值，有效权重 $1/(2\sigma_k^2)$ 反而适度提升，从而在不需要手动调参的前提下自动为该低方差任务找到合适的学习信号强度。这一自适应特性使得不确定性加权策略尤其适合本研究中标签方差跨任务差异超过20倍的极端不均衡场景。
...
|---------|:---------:|:----------------:|:------:|:----:|:-----:|:-----:|:-----:|
| 唇形同步（lip\_sync） | 1,985 | 0 | 100.0% | 4.763 | 0.025 | 4.60 | 4.90 |
...
从表3-5可以观察到两个显著的数据特征：第一，唇形同步维度的标签方差极低（标准差仅0.025，评分集中于4.6–4.9的窄区间），这意味着该维度在模型训练中几乎不具有有效的区分度，需要通过专门的权重策略进行处理（详见3.2.3节动态权重策略设计）；第二，表情自然度、跨模态一致性和整体感知质量三个维度存在约27\%–28\%的标签缺失（标记为-1.0），本研究通过标签掩码机制在训练时排除这些无效样本，确保损失计算的准确性。
...
第四，唇形同步（lip\_sync）维度的Pearson $r$为0.554，$R^2$为0.287。考虑到该维度原始标签方差极低（表3-5中标准差仅为0.025，评分集中于4.6–4.9的窄区间）这一数据特征，模型在极有限的标签区分度下仍取得了一定的预测性能，说明不确定性加权策略在处理低方差标签任务中发挥了积极作用。同时，SyncNet提供的唇形同步分数作为辅助参考信息，在一定程度上弥补了该维度标签信息不足的问题。
...
在训练策略方面，不确定性加权策略（E0）相比等权策略（E7，$w_k=1.0$）带来了5.5\%的性能提升（从0.549提升至0.581）。该策略的优势主要体现在两个方面：一是通过学习各任务的同方差不确定性参数 $\sigma_k$ 自适应地调整各任务的损失权重，使标签方差极低的任务（如唇形同步维度，标准差仅0.025）获得较低的有效权重，避免其主导梯度更新方向；二是在训练过程中动态平衡了五个子任务之间的优化进度，缓解了多任务学习中常见的梯度冲突问题。
...
综合而言，本章的核心创新贡献体现在以下三个方面：（1）提出了面向说话人脸视频的多模态多任务联合质量评估框架，首次实现了唇形同步、表情自然度、音频质量、跨模态一致性和整体感知质量五个维度的端到端统一预测，突破了现有方法仅能进行单一维度评估的局限；（2）设计了基于面部关键点几何距离的轻量化动作单元近似估计方法，在无需引入额外外部工具的条件下实现了17个核心AU的有效提取；（3）将基于同方差不确定性的自适应权重机制应用于标签方差差异超过20倍的极端不均衡多任务场景，通过与标签掩码机制的协同设计，解决了低方差任务梯度信号被湮没的问题。
