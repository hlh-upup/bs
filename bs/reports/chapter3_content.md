...
对本研究数据集中各模态特征的统计分析进一步揭示了严重的数值尺度失衡现象。将各模态特征沿样本和时间维度展平后统计其全局数值范围，四类特征的数值范围差异超过三个数量级：视觉特征（经ResNet101卷积后的激活值）约为[0, 14210]，音频特征（HuBERT隐藏状态值）约为[-6741, 6749]，关键点特征（MediaPipe归一化坐标经全样本展平后的统计范围）约为[-30142, 30142]，而动作单元特征（归一化后的AU强度值）约为[-941, 941]。需要说明的是，MediaPipe Face Mesh输出的单帧归一化坐标值域为$[0, 1]$，上述关键点特征的大数值范围源于后续Z-score标准化处理后的统计结果。
...
基于上述三个问题的分析，本章提出构建一个基于跨模态Transformer的多模态多任务学习框架，整体技术路线围绕三个层面展开：（1）针对异构特征融合难题，通过标准化预处理与PCA降维消除尺度差异与特征冗余，并设计模态嵌入与多头自注意力机制实现跨模态深层交互；（2）针对多任务优化难题，引入基于同方差不确定性的自适应权重策略与标签掩码机制，动态协调各任务的训练进度；（3）针对高维空间训练难题，综合运用梯度检查点、混合精度训练和自适应学习率调度等技术提升收敛效率与泛化性能。
...
>
> 图示说明：该图应从左至右展示四个核心阶段的数据流。左侧为输入端（原始视频），依次经过：（1）多模态特征提取阶段——四路并行的特征提取器（ResNet101→2048维、HuBERT→768维、MediaPipe→1404维、AU计算→17维），总维度4237；（2）特征预处理与降维阶段——NaN插补→Z-score标准化→PCA降维，总维度压缩至367维；（3）跨模态Transformer融合编码阶段——四路嵌入网络→模态嵌入→均值融合→可学习位置编码→6层16头Transformer编码器→全局平均池化→512维融合向量；（4）多任务质量预测阶段——五个独立预测头分别输出lip\_sync、expression、audio\_quality、cross\_modal、overall五个维度的\[1.0, 5.0\]评分。右侧标注动态任务权重与标签掩码机制。

**（1）多模态特征提取阶段**：从原始视频中并行提取视觉（ResNet101, 2048维）、音频（HuBERT, 768维）、面部关键点（MediaPipe, 1404维）和动作单元（几何距离, 17维）四类帧级时序特征，总维度为4237维。此外，SyncNet模块计算音视频同步置信度作为辅助参考信息。

**（2）特征预处理与降维阶段**：依次执行缺失值中位数插补、Z-score标准化和PCA降维，将总维度从4237维压缩至367维（降维率91.3%），各步骤的具体实现详见3.2.2节。

**（3）跨模态Transformer融合编码阶段**：四路模态嵌入网络将各模态特征映射至统一的512维语义空间，叠加可学习的模态嵌入和位置编码后，送入6层16头Transformer编码器进行深层语义编码，输出经全局平均池化聚合为512维的融合特征向量。该阶段的详细设计见3.2.3节。
...
面部动作单元（Action Unit, AU）是面部动作编码系统（FACS）中定义的基本面部肌肉运动单位，以标准化方式量化描述面部表情的细微变化。传统方法通常依赖OpenFace等外部工具链提取AU特征，但此类工具存在部署复杂、模型体积较大且无法与主框架联合优化等局限性。

为简化系统部署，本研究采用一种基于MediaPipe Face Mesh关键点的几何距离计算方案，利用已有的468个面部关键点，通过关键点对之间的距离与逆距离关系来近似估计17个核心动作单元的激活强度。该方法无需引入额外的预训练模型，降低了系统部署的复杂度。需要指出的是，基于几何距离的AU近似估计在精度上不及专用AU识别模型（如OpenFace），但在本研究的质量评估框架中，AU特征主要作为面部动态信息的辅助补充（消融实验显示其独立贡献约为4.0%），几何距离方案在精度与部署成本之间取得了合理的折中。
...
经过上述多模态特征提取后，每个视频样本被表示为四组时序特征矩阵：视觉特征 $\mathbf{V} \in \mathbb{R}^{150 \times 2048}$、音频特征 $\mathbf{A} \in \mathbb{R}^{150 \times 768}$、关键点特征 $\mathbf{K} \in \mathbb{R}^{150 \times 1404}$ 和动作单元特征 $\mathbf{U} \in \mathbb{R}^{150 \times 17}$。为解决3.1节分析的尺度失衡与高维冗余问题，需要在送入融合编码器之前进行预处理与降维。
...
经过3.2.2节的特征提取与预处理后，系统获得了四类经PCA降维的标准化时序特征。本节阐述跨模态Transformer融合编码器与多任务预测头的设计。图3-3展示了该模块的整体架构。

> **【图3-3 跨模态融合编码与多任务预测模块架构图】**
>
> 图示说明：该图应展示从降维后特征输入到质量评分输出的完整数据流。左侧为四路降维后特征输入（视觉100维、音频200维、关键点50维、AU 17维），分别经过四路并行嵌入网络（各含两层全连接+ReLU+Dropout）映射至统一的512维语义空间，并加入可学习的模态嵌入向量标识各模态来源。中部展示融合过程：四路嵌入特征经均值池化→叠加可学习位置编码→送入6层Transformer编码器（每层含16头多头自注意力+FFN+残差连接+层归一化），编码器输出经全局平均池化得到512维融合向量。右侧展示五个独立的任务预测头（512→256→128→1→Sigmoid→\[1.0,5.0\]），分别输出五个维度的质量评分。底部标注动态权重策略（固定权重/不确定性加权/GradNorm）和标签掩码机制。
...
在算法设计层面，本章构建了"多模态特征提取→特征预处理与PCA降维→跨模态Transformer融合编码→多任务质量预测"的端到端处理管线，通过PCA将原始4,237维特征压缩至367维，经跨模态Transformer实现深层特征交互，并借助不确定性加权策略实现五维质量评分的联合预测。

在实验验证层面，基于EmotionTalk数据集（1,985个样本）的实验结果表明，模型在五个评估维度上的平均Pearson相关系数为0.581，其中整体感知质量维度最优（$r$=0.698，CCC=0.651）。消融实验证实了Transformer跨模态融合编码器（贡献20.5%）、音频特征（14.5%）和PCA降维（11.9%）是性能提升的三个主要因素。

本章的核心创新贡献体现在两个方面：（1）首次将说话人脸视频质量评估建模为涵盖五个维度的多任务回归问题，突破了现有方法仅能进行单一维度评估的局限；（2）提出了基于同方差不确定性的自适应动态权重策略，有效缓解了多任务联合优化中的梯度冲突与负迁移问题。本章的算法设计与实验验证为第4章的系统集成与部署提供了技术基础。
