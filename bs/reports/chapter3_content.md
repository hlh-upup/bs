...
算法的第三阶段负责将降维后的四类异构模态特征映射至统一的语义空间，并通过深层注意力机制实现跨模态信息的交互融合。具体而言，四路模态嵌入网络分别将各模态特征映射至统一的512维语义空间，并叠加可学习的模态嵌入向量以标识各模态来源，经逐元素平均融合并叠加可学习的位置编码后，送入6层16头Transformer编码器进行深层语义编码。编码器输出的时序特征经全局平均池化聚合为512维的全局融合特征向量，供后续预测头使用。该阶段的详细设计见3.2.3节。
...
如3.1.2节所分析，五个评估任务在多任务联合优化过程中面临梯度冲突与训练进度不一致的挑战。不同维度衡量的质量属性差异较大，各任务的最优梯度方向可能在共享参数空间中产生冲突；此外，各任务的优化难度和收敛速度存在固有差异，固定等权重策略难以适应训练过程中各任务状态的动态变化。为解决这一问题，本研究设计并对比了以下三种动态任务权重策略。
