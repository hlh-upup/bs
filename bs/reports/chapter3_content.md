...
第四，在实验结果及分析层面（3.4节），本章从整体性能评估、消融实验和预测误差分析三个方面全面展示并深入分析了实验结果。实验表明，本文所提出的多模态多任务评估模型在整体感知质量维度上达到了0.698的Pearson相关系数，在音频质量维度上达到了0.621，在跨模态一致性维度上达到了0.596，验证了所提方法在多维度视频质量评估任务上的有效性。
...
在实验验证层面，基于EmotionTalk数据集（1,985个样本）的实验结果表明，本章所提出的模型在五个评估维度上均取得了有效的预测性能。其中，整体感知质量维度达到了0.698的Pearson相关系数和0.651的一致性相关系数（CCC），音频质量维度达到了0.621的Pearson $r$，跨模态一致性维度达到了0.596的Pearson $r$。消融实验系统性地量化了各关键模块的贡献，结果表明Transformer跨模态融合编码器（贡献20.5\%）、音频特征（贡献14.5\%）和PCA降维（贡献11.9\%）是性能提升的三个最重要因素。
