...
此外，数据集中部分维度存在标签缺失的情况，需要在损失计算中加以处理。统计分析表明，唇形同步维度由SyncNet自动提取，标签完整率为100%；表情自然度的有效标签比例为72.8%，音频质量为77.8%，跨模态一致性和整体感知质量均为72.6%，其余样本的标签以无效值（-1.0）标记。这意味着不同任务在每个训练批次中的有效样本数量可能存在差异，需要通过掩码机制妥善处理。因此，如何设计自适应的动态任务权重策略，使模型能够在多任务联合优化中有效缓解梯度冲突、协调各任务的训练进度，是本章面临的第二个问题。
...
在标注方面，EmotionTalk数据集提供了五个评估维度的质量标注，其中表情自然度（expression）、音频质量（audio\_quality）、跨模态一致性（cross\_modal）和整体感知质量（overall）四个维度采用人工主观评分标注。唇形同步质量（lip\_sync）维度则采用基于SyncNet通用模型的自动化提取方式获得：本研究利用预训练的SyncNet模型对EmotionTalk数据集中的视频进行音视频同步置信度检测，并将同步置信度经Sigmoid映射转换为$[1.0, 5.0]$评分区间的唇形同步分数（具体映射方法见3.2.1节）。选择该方式的原因在于，唇形同步质量主要反映口型运动与语音信号之间的物理时序对齐关系，其评判标准相对客观，适合采用基于信号处理的自动化工具进行量化。此外，为增强数据集在生成方法层面的多样性与代表性，本研究在EmotionTalk原始样本的基础上补充了部分自行制作的AI生成说话人脸视频样本，这些补充样本同样经过统一的特征提取与标注流程处理。五个评估维度的评分范围均为1.0（最差）至5.0（最优）。表3-5展示了各维度标签的详细统计信息。

**表3-5 EmotionTalk数据集各维度标签统计**

| 评估维度 | 有效标签数 | 无效标签数（-1.0） | 有效比例 | 均值 | 标准差 | 最小值 | 最大值 |
|---------|:---------:|:----------------:|:------:|:----:|:-----:|:-----:|:-----:|
| 唇形同步（lip\_sync）$^\dagger$ | 1,985 | 0 | 100.0% | 3.58 | 0.53 | 1.0 | 5.0 |
...
从表3-5可以观察到以下数据特征：第一，五个评估维度的标签分布总体均衡，各维度均值分布在3.18–3.58之间，标准差在0.52–0.61之间，评分覆盖了1.0–5.0的完整区间，表明数据集中的视频样本在各质量维度上均具有良好的区分度；第二，唇形同步维度（标注$^\dagger$）的标签由SyncNet模型自动提取，因此有效率为100%且无缺失；其余四个维度采用人工标注，表情自然度、跨模态一致性和整体感知质量三个维度存在部分标签缺失（标记为-1.0），本研究通过标签掩码机制在训练时排除这些无效样本，确保损失计算的准确性。
...
第四，唇形同步（lip\_sync）维度的Pearson $r$为0.554，$R^2$为0.287。需要指出的是，该维度的标签由SyncNet模型自动提取而非人工标注，因此模型本质上是在学习拟合SyncNet的输出分布。唇形同步质量的评估需要精确捕捉口型运动与语音信号之间的时序对齐关系，对模型的时序建模能力要求较高。面部关键点特征中口唇区域关键点的运动模式为该维度的预测提供了显式的几何线索，有助于模型学习到与SyncNet同步检测相一致的质量评判模式。

第五，表情自然度（expression）维度表现相对较弱，Pearson $r$为0.437，$R^2$仅为0.173。分析其原因，主要在于表情自然度的主观感知具有较高的个体差异性，不同评估者对"自然"与"不自然"表情的判定标准存在较大分歧，导致标签本身包含较多的评估噪声。此外，表情自然度涉及面部肌肉运动的微妙时序变化，其细粒度特征相比其他维度更难以通过帧级特征有效捕捉。
