...
class Preprocesser:
    ...
    def crop(self, img_np_list, still=False, xsize=512):    # first frame for all video
        ...
        if lm is None:
            # 改为标准异常类型，避免 TypeError: exceptions must derive from BaseException
            raise RuntimeError('can not detect the landmark from source image (未检测到人脸关键点，请检查上传图片是否清晰、正脸且包含完整五官)')
