...
#当前的路径
exp_root = os.path.join(now_dir, exp_root)
...
SoVITS_weight_root= os.path.join(now_dir, "SoVITS_weights")
GPT_weight_root=os.path.join(now_dir,"GPT_weights")
...
def open1Ba(batch_size,total_epoch,exp_name,text_low_lr_rate,if_save_latest,if_save_every_weights,save_every_epoch,gpu_numbers1Ba,pretrained_s2G,pretrained_s2D):
    ...
    if(p_train_SoVITS==None):
        s2josn = os.path.join(now_dir, "GPT_SoVITS/configs/s2.json")
        with open(s2josn)as f:
            data=f.read()
        ...
        #删除文件夹
        s2_model_dir = os.path.join(s2_dir, "logs_s2")
        shutil.rmtree(s2_model_dir)
    else:
        yield "已有正在进行的SoVITS训练任务，需先终止才能开启下一次任务",{"__type__":"update","visible":False},{"__type__":"update","visible":True}

def open1Ba_Train(batch_size,total_epoch,exp_name,text_low_lr_rate,if_save_latest,if_save_every_weights,save_every_epoch,gpu_numbers1Ba,pretrained_s2G,pretrained_s2D):
    #batch_size : 3
    #total_epoch :8
    #exp_name : "Hui"
    #text_low_lr_rate : 0.4
    ...
    if(p_train_SoVITS==None):
        ...
        tmp_config_path="%s/tmp_s2.json"%tmp
        with open(tmp_config_path,"w")as f:f.write(json.dumps(data))
        file = os.path.join(now_dir, "GPT_SoVITS/s2_train.py")
        cmd = '"%s" %s --config "%s"'%(python_exec, file, tmp_config_path)
        print(cmd)
        p_train_SoVITS = Popen(cmd, shell=True)
        p_train_SoVITS.wait()
        p_train_SoVITS=None
...
def open1Bb(batch_size,total_epoch,exp_name,if_dpo,if_save_latest,if_save_every_weights,save_every_epoch,gpu_numbers,pretrained_s1):
    global p_train_GPT
    s1longer_path = os.path.join(now_dir, "GPT_SoVITS/configs/s1longer.yaml")
    if(p_train_GPT==None):
        with open(s1longer_path)as f:
            data=f.read()
        ...
        p_train_GPT=None
                #删除文件夹
        s2_model_dir = os.path.join(s1_dir, "logs_s1")
        shutil.rmtree(s2_model_dir)
        yield "GPT训练完成",{"__type__":"update","visible":True},{"__type__":"update","visible":False}
    else:
        yield "已有正在进行的GPT训练任务，需先终止才能开启下一次任务",{"__type__":"update","visible":False},{"__type__":"update","visible":True}

def open1Bb_Train(batch_size,total_epoch,exp_name,if_dpo,if_save_latest,if_save_every_weights,save_every_epoch,gpu_numbers,pretrained_s1):
    global p_train_GPT
    s1longer_path = os.path.join(now_dir, "GPT_SoVITS/configs/s1longer.yaml")
    if(p_train_GPT==None):
        with open(s1longer_path)as f:
        ...
        with open(tmp_config_path, "w") as f:f.write(yaml.dump(data, default_flow_style=False))
        # cmd = '"%s" GPT_SoVITS/s1_train.py --config_file "%s" --train_semantic_path "%s/6-name2semantic.tsv" --train_phoneme_path "%s/2-name2text.txt" --output_dir "%s/logs_s1"'%(python_exec,tmp_config_path,s1_dir,s1_dir,s1_dir)
        file = os.path.join(now_dir,"GPT_SoVITS/s1_train.py")
        cmd = '"%s" %s --config_file "%s" '%(python_exec, file, tmp_config_path)
        print(cmd)
        p_train_GPT = Popen(cmd, shell=True)
        p_train_GPT.wait()
        p_train_GPT=None
...
def open1abc(inp_text,inp_wav_dir,exp_name,gpu_numbers1a,gpu_numbers1Ba,gpu_numbers1c,bert_pretrained_dir,ssl_pretrained_dir,pretrained_s2G_path):
    #inp_text ： 'C:\\Users\\XiaoHui\\Desktop\\man\\list\\split.list'
    #inp_wav_dir ： 'C:\\Users\\XiaoHui\\Desktop\\man\\slicer_opt'
    #exp_name ： 'Hui'
    #gpu_numbers1a ： '0-0'
    #gpu_numbers1Ba ： '0-0'
    #gpu_numbers1c ： '0-0'
...
def open1abc_Train(inp_text,inp_wav_dir,exp_name,gpu_numbers1a,gpu_numbers1Ba,gpu_numbers1c,bert_pretrained_dir,ssl_pretrained_dir,pretrained_s2G_path):
    #inp_text ： 'C:\\Users\\XiaoHui\\Desktop\\man\\list\\split.list'
    #inp_wav_dir ： 'C:\\Users\\XiaoHui\\Desktop\\man\\slicer_opt'
    #exp_name ： 'Hui'
    #gpu_numbers1a ： '0-0'
        ...
        try:
            ...
            if(os.path.exists(path_text)==False or (os.path.exists(path_text)==True and len(open(path_text,"r",encoding="utf8").read().strip("\n").split("\n"))<2)):
                ...
                for i_part in range(all_parts):
                    ...
                    os.environ.update(config)
                    file = os.path.join(now_dir,"GPT_SoVITS/prepare_datasets/1-get-text.py")
                    cmd = '"%s" %s'%(python_exec, file)
                    print(cmd)
                    p = Popen(cmd, shell=True)
                    ps1abc.append(p)
            ...
            for i_part in range(all_parts):
                ...
                os.environ.update(config)
                file = os.path.join(now_dir,"GPT_SoVITS/prepare_datasets/2-get-hubert-wav32k.py")
                cmd = '"%s" %s'%(python_exec, file)
                print(cmd)
                p = Popen(cmd, shell=True)
                ps1abc.append(p)
            ...
            path_semantic = "%s/6-name2semantic.tsv" % opt_dir
            if(os.path.exists(path_semantic)==False or (os.path.exists(path_semantic)==True and os.path.getsize(path_semantic)<31)):
                json_path = os.path.join(now_dir,"GPT_SoVITS/configs/s2.json")
                config={
                    "inp_text":inp_text,
                    "exp_name":exp_name,
                    "opt_dir":opt_dir,
                    "pretrained_s2G":pretrained_s2G_path,
                    "s2config_path":json_path,
                }
                gpu_names=gpu_numbers1c.split("-")
                all_parts=len(gpu_names)
                for i_part in range(all_parts):
                    ...
                    os.environ.update(config)
                    file = os.path.join(now_dir,"GPT_SoVITS/prepare_datasets/3-get-semantic.py")
                    cmd = '"%s" %s'%(python_exec, file)
                    print(cmd)
                    p = Popen(cmd, shell=True)
                    ps1abc.append(p)
